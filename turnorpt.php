...
<?php include_once "turnorptinfo.php" ?>
<?php
...
$turno_rpt = NULL; // Initialize page object first

class crturno_rpt extends crturno {
	...
	function __construct() {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'turno', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fturnorpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'turno');
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_turno\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_turno',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fturnorpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fturnorpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fturnorpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['turno'] = ewr_Conv($rs->fields('turno'), 200);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			...
			$this->Val[2] = $this->turno->CurrentValue;
		} else {
			$this->id->setDbValue("");
			$this->turno->setDbValue("");
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->turno->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// turno
			$CurrentValue = $this->turno->CurrentValue;
			$ViewValue = &$this->turno->ViewValue;
			$ViewAttrs = &$this->turno->ViewAttrs;
			$CellAttrs = &$this->turno->CellAttrs;
			$HrefValue = &$this->turno->HrefValue;
			$LinkAttrs = &$this->turno->LinkAttrs;
			$this->Cell_Rendered($this->turno, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->turno->Visible) $this->DtlColumnCount += 1;
	}
...
// Create page object
if (!isset($turno_rpt)) $turno_rpt = new crturno_rpt();
...
// Create page object
var turno_rpt = new ewr_Page("turno_rpt");

// Page properties
turno_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = turno_rpt.PageID;
</script>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_turno" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id"><div class="turno_id"><span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id">
<?php if ($Page->SortUrl($Page->id) == "") { ?>
		<div class="ewTableHeaderBtn turno_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer turno_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->turno->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="turno"><div class="turno_turno"><span class="ewTableHeaderCaption"><?php echo $Page->turno->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="turno">
<?php if ($Page->SortUrl($Page->turno) == "") { ?>
		<div class="ewTableHeaderBtn turno_turno">
			<span class="ewTableHeaderCaption"><?php echo $Page->turno->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer turno_turno" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->turno) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->turno->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->turno->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->turno->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php } ?>
<?php if ($Page->turno->Visible) { ?>
		<td data-field="turno"<?php echo $Page->turno->CellAttributes() ?>>
<span<?php echo $Page->turno->ViewAttributes() ?>><?php echo $Page->turno->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
