...
$apoderado_list = NULL; // Initialize page object first

class capoderado_list extends capoderado {
	...
	// Page object name
	var $PageObjName = 'apoderado_list';

	// Grid form hidden field names
	var $FormName = 'fapoderadolist';
	...
	function __construct() {
		...
		$this->AddUrl = "apoderadoadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "apoderadodelete.php";
		$this->MultiUpdateUrl = "apoderadoupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fapoderadolistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fapoderadolistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fapoderadolistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fapoderadolistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fapoderadolistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fapoderadolist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fapoderadolistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadSearchValues() {
		...
		// parentesco
		$this->parentesco->AdvancedSearch->SearchValue = @$_GET["x_parentesco"];
		if ($this->parentesco->AdvancedSearch->SearchValue <> "" && $this->Command == "") $this->Command = "search";
...
// Create page object
if (!isset($apoderado_list)) $apoderado_list = new capoderado_list();

// Page init
$apoderado_list->Page_Init();

// Page main
$apoderado_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$apoderado_list->Page_Render();
?>
...
var CurrentForm = fapoderadolist = new ew_Form("fapoderadolist", "list");
fapoderadolist.FormKeyCountName = '<?php echo $apoderado_list->FormKeyCountName ?>';

// Form_CustomValidate event
fapoderadolist.Form_CustomValidate = 
...
fapoderadolist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fapoderadolist.Lists["x_sexo"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fapoderadolist.Lists["x_sexo"].Options = <?php echo json_encode($apoderado_list->sexo->Options()) ?>;

// Form object for search
var CurrentSearchForm = fapoderadolistsrch = new ew_Form("fapoderadolistsrch");

// Validate function for search
fapoderadolistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
fapoderadolistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fapoderadolistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php if ($apoderado_list->TotalRecs > 0 && $apoderado_list->ExportOptions->Visible()) { ?>
<?php $apoderado_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($apoderado_list->SearchOptions->Visible()) { ?>
<?php $apoderado_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($apoderado_list->FilterOptions->Visible()) { ?>
<?php $apoderado_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php
	$bSelectLimit = $apoderado_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($apoderado_list->TotalRecs <= 0)
			$apoderado_list->TotalRecs = $apoderado->ListRecordCount();
	} else {
		if (!$apoderado_list->Recordset && ($apoderado_list->Recordset = $apoderado_list->LoadRecordset()))
			$apoderado_list->TotalRecs = $apoderado_list->Recordset->RecordCount();
	}
	$apoderado_list->StartRec = 1;
	if ($apoderado_list->DisplayRecs <= 0 || ($apoderado->Export <> "" && $apoderado->ExportAll)) // Display all records
		$apoderado_list->DisplayRecs = $apoderado_list->TotalRecs;
	if (!($apoderado->Export <> "" && $apoderado->ExportAll))
		$apoderado_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$apoderado_list->Recordset = $apoderado_list->LoadRecordset($apoderado_list->StartRec-1, $apoderado_list->DisplayRecs);

	// Set no record found message
	if ($apoderado->CurrentAction == "" && $apoderado_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$apoderado_list->setWarningMessage(ew_DeniedMsg());
		if ($apoderado_list->SearchWhere == "0=101")
			$apoderado_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$apoderado_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$apoderado_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($apoderado->Export == "" && $apoderado->CurrentAction == "") { ?>
<form name="fapoderadolistsrch" id="fapoderadolistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($apoderado_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fapoderadolistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="apoderado">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$apoderado_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$apoderado->RowType = EW_ROWTYPE_SEARCH;

// Render row
$apoderado->ResetAttrs();
$apoderado_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($apoderado->parentesco->Visible) { // parentesco ?>
	<div id="xsc_parentesco" class="ewCell form-group">
		<label for="x_parentesco" class="ewSearchCaption ewLabel"><?php echo $apoderado->parentesco->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_parentesco" id="z_parentesco" value="LIKE"></span>
...
<?php if ($apoderado->apellidopaterno->Visible) { // apellidopaterno ?>
	<div id="xsc_apellidopaterno" class="ewCell form-group">
		<label for="x_apellidopaterno" class="ewSearchCaption ewLabel"><?php echo $apoderado->apellidopaterno->FldCaption() ?></label>
...
<?php if ($apoderado->apellidomaterno->Visible) { // apellidomaterno ?>
	<div id="xsc_apellidomaterno" class="ewCell form-group">
		<label for="x_apellidomaterno" class="ewSearchCaption ewLabel"><?php echo $apoderado->apellidomaterno->FldCaption() ?></label>
...
<?php if ($apoderado->nombres->Visible) { // nombres ?>
	<div id="xsc_nombres" class="ewCell form-group">
		<label for="x_nombres" class="ewSearchCaption ewLabel"><?php echo $apoderado->nombres->FldCaption() ?></label>
...
<?php if ($apoderado->ci->Visible) { // ci ?>
	<div id="xsc_ci" class="ewCell form-group">
		<label for="x_ci" class="ewSearchCaption ewLabel"><?php echo $apoderado->ci->FldCaption() ?></label>
...
<?php } ?>
<?php } ?>
<?php $apoderado_list->ShowPageHeader(); ?>
<?php
$apoderado_list->ShowMessage();
?>
<?php if ($apoderado_list->TotalRecs > 0 || $apoderado->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($apoderado_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> apoderado">
<div class="box-header ewGridUpperPanel">
<?php if ($apoderado->CurrentAction <> "gridadd" && $apoderado->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($apoderado_list->Pager)) $apoderado_list->Pager = new cPrevNextPager($apoderado_list->StartRec, $apoderado_list->DisplayRecs, $apoderado_list->TotalRecs, $apoderado_list->AutoHidePager) ?>
<?php if ($apoderado_list->Pager->RecordCount > 0 && $apoderado_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($apoderado_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $apoderado_list->PageUrl() ?>start=<?php echo $apoderado_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($apoderado_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $apoderado_list->PageUrl() ?>start=<?php echo $apoderado_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $apoderado_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($apoderado_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $apoderado_list->PageUrl() ?>start=<?php echo $apoderado_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($apoderado_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $apoderado_list->PageUrl() ?>start=<?php echo $apoderado_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $apoderado_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($apoderado_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $apoderado_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $apoderado_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $apoderado_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($apoderado_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
</div>
<form name="fapoderadolist" id="fapoderadolist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($apoderado_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $apoderado_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="apoderado">
<div id="gmp_apoderado" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($apoderado_list->TotalRecs > 0 || $apoderado->CurrentAction == "gridedit") { ?>
<table id="tbl_apoderadolist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$apoderado_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$apoderado_list->RenderListOptions();

// Render list options (header, left)
$apoderado_list->ListOptions->Render("header", "left");
?>
<?php if ($apoderado->id->Visible) { // id ?>
	<?php if ($apoderado->SortUrl($apoderado->id) == "") { ?>
		<th data-name="id" class="<?php echo $apoderado->id->HeaderCellClass() ?>"><div id="elh_apoderado_id" class="apoderado_id"><div class="ewTableHeaderCaption"><?php echo $apoderado->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id" class="<?php echo $apoderado->id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->id) ?>',2);"><div id="elh_apoderado_id" class="apoderado_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->parentesco->Visible) { // parentesco ?>
	<?php if ($apoderado->SortUrl($apoderado->parentesco) == "") { ?>
		<th data-name="parentesco" class="<?php echo $apoderado->parentesco->HeaderCellClass() ?>"><div id="elh_apoderado_parentesco" class="apoderado_parentesco"><div class="ewTableHeaderCaption"><?php echo $apoderado->parentesco->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="parentesco" class="<?php echo $apoderado->parentesco->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->parentesco) ?>',2);"><div id="elh_apoderado_parentesco" class="apoderado_parentesco">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->parentesco->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->parentesco->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->parentesco->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->apellidopaterno->Visible) { // apellidopaterno ?>
	<?php if ($apoderado->SortUrl($apoderado->apellidopaterno) == "") { ?>
		<th data-name="apellidopaterno" class="<?php echo $apoderado->apellidopaterno->HeaderCellClass() ?>"><div id="elh_apoderado_apellidopaterno" class="apoderado_apellidopaterno"><div class="ewTableHeaderCaption"><?php echo $apoderado->apellidopaterno->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="apellidopaterno" class="<?php echo $apoderado->apellidopaterno->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->apellidopaterno) ?>',2);"><div id="elh_apoderado_apellidopaterno" class="apoderado_apellidopaterno">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->apellidopaterno->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->apellidopaterno->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->apellidopaterno->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->apellidomaterno->Visible) { // apellidomaterno ?>
	<?php if ($apoderado->SortUrl($apoderado->apellidomaterno) == "") { ?>
		<th data-name="apellidomaterno" class="<?php echo $apoderado->apellidomaterno->HeaderCellClass() ?>"><div id="elh_apoderado_apellidomaterno" class="apoderado_apellidomaterno"><div class="ewTableHeaderCaption"><?php echo $apoderado->apellidomaterno->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="apellidomaterno" class="<?php echo $apoderado->apellidomaterno->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->apellidomaterno) ?>',2);"><div id="elh_apoderado_apellidomaterno" class="apoderado_apellidomaterno">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->apellidomaterno->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->apellidomaterno->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->apellidomaterno->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->nombres->Visible) { // nombres ?>
	<?php if ($apoderado->SortUrl($apoderado->nombres) == "") { ?>
		<th data-name="nombres" class="<?php echo $apoderado->nombres->HeaderCellClass() ?>"><div id="elh_apoderado_nombres" class="apoderado_nombres"><div class="ewTableHeaderCaption"><?php echo $apoderado->nombres->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nombres" class="<?php echo $apoderado->nombres->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->nombres) ?>',2);"><div id="elh_apoderado_nombres" class="apoderado_nombres">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->nombres->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->nombres->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->nombres->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->ci->Visible) { // ci ?>
	<?php if ($apoderado->SortUrl($apoderado->ci) == "") { ?>
		<th data-name="ci" class="<?php echo $apoderado->ci->HeaderCellClass() ?>"><div id="elh_apoderado_ci" class="apoderado_ci"><div class="ewTableHeaderCaption"><?php echo $apoderado->ci->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="ci" class="<?php echo $apoderado->ci->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->ci) ?>',2);"><div id="elh_apoderado_ci" class="apoderado_ci">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->ci->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->ci->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->ci->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->fechanacimiento->Visible) { // fechanacimiento ?>
	<?php if ($apoderado->SortUrl($apoderado->fechanacimiento) == "") { ?>
		<th data-name="fechanacimiento" class="<?php echo $apoderado->fechanacimiento->HeaderCellClass() ?>"><div id="elh_apoderado_fechanacimiento" class="apoderado_fechanacimiento"><div class="ewTableHeaderCaption"><?php echo $apoderado->fechanacimiento->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="fechanacimiento" class="<?php echo $apoderado->fechanacimiento->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->fechanacimiento) ?>',2);"><div id="elh_apoderado_fechanacimiento" class="apoderado_fechanacimiento">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->fechanacimiento->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->fechanacimiento->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->fechanacimiento->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->sexo->Visible) { // sexo ?>
	<?php if ($apoderado->SortUrl($apoderado->sexo) == "") { ?>
		<th data-name="sexo" class="<?php echo $apoderado->sexo->HeaderCellClass() ?>"><div id="elh_apoderado_sexo" class="apoderado_sexo"><div class="ewTableHeaderCaption"><?php echo $apoderado->sexo->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="sexo" class="<?php echo $apoderado->sexo->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->sexo) ?>',2);"><div id="elh_apoderado_sexo" class="apoderado_sexo">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->sexo->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->sexo->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->sexo->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($apoderado->celular->Visible) { // celular ?>
	<?php if ($apoderado->SortUrl($apoderado->celular) == "") { ?>
		<th data-name="celular" class="<?php echo $apoderado->celular->HeaderCellClass() ?>"><div id="elh_apoderado_celular" class="apoderado_celular"><div class="ewTableHeaderCaption"><?php echo $apoderado->celular->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="celular" class="<?php echo $apoderado->celular->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->celular) ?>',2);"><div id="elh_apoderado_celular" class="apoderado_celular">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->celular->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->celular->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->celular->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($apoderado->ocupaciF3n->Visible) { // ocupación ?>
...
<?php if ($apoderado->observaciones->Visible) { // observaciones ?>
	<?php if ($apoderado->SortUrl($apoderado->observaciones) == "") { ?>
		<th data-name="observaciones" class="<?php echo $apoderado->observaciones->HeaderCellClass() ?>"><div id="elh_apoderado_observaciones" class="apoderado_observaciones"><div class="ewTableHeaderCaption"><?php echo $apoderado->observaciones->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="observaciones" class="<?php echo $apoderado->observaciones->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $apoderado->SortUrl($apoderado->observaciones) ?>',2);"><div id="elh_apoderado_observaciones" class="apoderado_observaciones">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $apoderado->observaciones->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($apoderado->observaciones->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($apoderado->observaciones->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$apoderado_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($apoderado->ExportAll && $apoderado->Export <> "") {
	$apoderado_list->StopRec = $apoderado_list->TotalRecs;
} else {

	// Set the last record to display
	if ($apoderado_list->TotalRecs > $apoderado_list->StartRec + $apoderado_list->DisplayRecs - 1)
		$apoderado_list->StopRec = $apoderado_list->StartRec + $apoderado_list->DisplayRecs - 1;
	else
		$apoderado_list->StopRec = $apoderado_list->TotalRecs;
}
$apoderado_list->RecCnt = $apoderado_list->StartRec - 1;
if ($apoderado_list->Recordset && !$apoderado_list->Recordset->EOF) {
	$apoderado_list->Recordset->MoveFirst();
	$bSelectLimit = $apoderado_list->UseSelectLimit;
	if (!$bSelectLimit && $apoderado_list->StartRec > 1)
		$apoderado_list->Recordset->Move($apoderado_list->StartRec - 1);
} elseif (!$apoderado->AllowAddDeleteRow && $apoderado_list->StopRec == 0) {
	$apoderado_list->StopRec = $apoderado->GridAddRowCount;
}

// Initialize aggregate
$apoderado->RowType = EW_ROWTYPE_AGGREGATEINIT;
$apoderado->ResetAttrs();
$apoderado_list->RenderRow();
while ($apoderado_list->RecCnt < $apoderado_list->StopRec) {
	$apoderado_list->RecCnt++;
	if (intval($apoderado_list->RecCnt) >= intval($apoderado_list->StartRec)) {
		$apoderado_list->RowCnt++;

		// Set up key count
		$apoderado_list->KeyCount = $apoderado_list->RowIndex;

		// Init row class and style
		$apoderado->ResetAttrs();
		$apoderado->CssClass = "";
		if ($apoderado->CurrentAction == "gridadd") {
		} else {
			$apoderado_list->LoadRowValues($apoderado_list->Recordset); // Load row values
		}
		$apoderado->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$apoderado->RowAttrs = array_merge($apoderado->RowAttrs, array('data-rowindex'=>$apoderado_list->RowCnt, 'id'=>'r' . $apoderado_list->RowCnt . '_apoderado', 'data-rowtype'=>$apoderado->RowType));
...
$apoderado_list->ListOptions->Render("body", "left", $apoderado_list->RowCnt);
?>
	<?php if ($apoderado->id->Visible) { // id ?>
		<td data-name="id"<?php echo $apoderado->id->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_id" class="apoderado_id">
...
<?php echo $apoderado->id->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->parentesco->Visible) { // parentesco ?>
		<td data-name="parentesco"<?php echo $apoderado->parentesco->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_parentesco" class="apoderado_parentesco">
...
<?php echo $apoderado->parentesco->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->apellidopaterno->Visible) { // apellidopaterno ?>
		<td data-name="apellidopaterno"<?php echo $apoderado->apellidopaterno->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_apellidopaterno" class="apoderado_apellidopaterno">
...
<?php echo $apoderado->apellidopaterno->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->apellidomaterno->Visible) { // apellidomaterno ?>
		<td data-name="apellidomaterno"<?php echo $apoderado->apellidomaterno->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_apellidomaterno" class="apoderado_apellidomaterno">
...
<?php echo $apoderado->apellidomaterno->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->nombres->Visible) { // nombres ?>
		<td data-name="nombres"<?php echo $apoderado->nombres->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_nombres" class="apoderado_nombres">
...
<?php echo $apoderado->nombres->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->ci->Visible) { // ci ?>
		<td data-name="ci"<?php echo $apoderado->ci->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_ci" class="apoderado_ci">
...
<?php echo $apoderado->ci->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->fechanacimiento->Visible) { // fechanacimiento ?>
		<td data-name="fechanacimiento"<?php echo $apoderado->fechanacimiento->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_fechanacimiento" class="apoderado_fechanacimiento">
...
<?php echo $apoderado->fechanacimiento->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->sexo->Visible) { // sexo ?>
		<td data-name="sexo"<?php echo $apoderado->sexo->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_sexo" class="apoderado_sexo">
...
<?php echo $apoderado->direcciF3n->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->celular->Visible) { // celular ?>
		<td data-name="celular"<?php echo $apoderado->celular->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_celular" class="apoderado_celular">
...
<?php echo $apoderado->ocupaciF3n->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($apoderado->observaciones->Visible) { // observaciones ?>
		<td data-name="observaciones"<?php echo $apoderado->observaciones->CellAttributes() ?>>
<span id="el<?php echo $apoderado_list->RowCnt ?>_apoderado_observaciones" class="apoderado_observaciones">
...
$apoderado_list->ListOptions->Render("body", "right", $apoderado_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($apoderado->CurrentAction <> "gridadd")
		$apoderado_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($apoderado->CurrentAction == "") { ?>
...
// Close recordset
if ($apoderado_list->Recordset)
	$apoderado_list->Recordset->Close();
?>
...
<?php } ?>
<?php if ($apoderado_list->TotalRecs == 0 && $apoderado->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($apoderado_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fapoderadolistsrch.FilterList = <?php echo $apoderado_list->GetFilterList() ?>;
fapoderadolistsrch.Init();
fapoderadolist.Init();
</script>
<?php
$apoderado_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
