...
class crviewestudiante_rpt extends crviewestudiante {
	...
	function Page_Init() {
		...
		$this->tipodiscapcidad->PlaceHolder = $this->tipodiscapcidad->FldCaption();
...
//		$item->Visible = TRUE;
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'viewestudiante_nrodiscapacidad')
			...
			if ($this->ClearExtFilter == 'viewestudiante_nombre')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'nombre');

			// Clear extended filter for field materno
			if ($this->ClearExtFilter == 'viewestudiante_materno')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'materno');

			// Clear extended filter for field paterno
			if ($this->ClearExtFilter == 'viewestudiante_paterno')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'paterno');

			// Clear extended filter for field edad
			if ($this->ClearExtFilter == 'viewestudiante_edad')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'edad');

			// Clear extended filter for field discapacidad
			if ($this->ClearExtFilter == 'viewestudiante_discapacidad')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'discapacidad');

			// Clear extended filter for field tipodiscapcidad
			if ($this->ClearExtFilter == 'viewestudiante_tipodiscapcidad')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'tipodiscapcidad');
		...
		// Reset search command
			...
			$this->SetSessionFilterValues($this->tipodiscapcidad->SearchValue, $this->tipodiscapcidad->SearchOperator, $this->tipodiscapcidad->SearchCondition, $this->tipodiscapcidad->SearchValue2, $this->tipodiscapcidad->SearchOperator2, 'tipodiscapcidad'); // Field tipodiscapcidad
			...
			if ($this->GetFilterValues($this->ci)) {
				if (ewr_FieldDataType($this->ci->FldType) == EWR_DATATYPE_DATE) // Format default date format
					$this->ci->SearchValue = ewr_FormatDateTime($this->ci->SearchValue, 0);
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetFilterValues($this->tipodiscapcidad)) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionFilterValues($this->tipodiscapcidad); // Field tipodiscapcidad
		}
		...
		$this->BuildExtendedFilter($this->tipodiscapcidad, $sFilter, FALSE, TRUE); // Field tipodiscapcidad
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_viewestudiante_codigorude'] = ($this->codigorude->SelectionList == "") ? EWR_INIT_VALUE : $this->codigorude->SelectionList;
			...
			$_SESSION['sel_viewestudiante_ci'] = ($this->ci->SelectionList == "") ? EWR_INIT_VALUE : $this->ci->SelectionList;
			...
			$_SESSION['sel_viewestudiante_fechanacimiento'] = ($this->fechanacimiento->SelectionList == "") ? EWR_INIT_VALUE : $this->fechanacimiento->SelectionList;
			...
			$_SESSION['sel_viewestudiante_nrodiscapacidad'] = ($this->nrodiscapacidad->SelectionList == "") ? EWR_INIT_VALUE : $this->nrodiscapacidad->SelectionList;
			...
			// Field materno
			$sWrk = "";
			$this->BuildExtendedFilter($this->materno, $sWrk);
			ewr_LoadSelectionFromFilter($this->materno, $sWrk, $this->materno->SelectionList);
			$_SESSION['sel_viewestudiante_materno'] = ($this->materno->SelectionList == "") ? EWR_INIT_VALUE : $this->materno->SelectionList;

			// Field paterno
			$sWrk = "";
			$this->BuildExtendedFilter($this->paterno, $sWrk);
			ewr_LoadSelectionFromFilter($this->paterno, $sWrk, $this->paterno->SelectionList);
			...
			$_SESSION['sel_viewestudiante_discapacidad'] = ($this->discapacidad->SelectionList == "") ? EWR_INIT_VALUE : $this->discapacidad->SelectionList;

			// Field tipodiscapcidad
			$sWrk = "";
			$this->BuildExtendedFilter($this->tipodiscapcidad, $sWrk);
			ewr_LoadSelectionFromFilter($this->tipodiscapcidad, $sWrk, $this->tipodiscapcidad->SelectionList);
			$_SESSION['sel_viewestudiante_tipodiscapcidad'] = ($this->tipodiscapcidad->SelectionList == "") ? EWR_INIT_VALUE : $this->tipodiscapcidad->SelectionList;
		}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		$sWrk = "";
		$this->BuildExtendedFilter($this->materno, $sWrk, TRUE);
		...
		$sWrk = "";
		$this->BuildExtendedFilter($this->paterno, $sWrk, TRUE);
		...
		// Field tipodiscapcidad
		$this->SetDefaultExtFilter($this->tipodiscapcidad, "LIKE", NULL, 'AND', "=", NULL);
		if (!$this->SearchCommand) $this->ApplyDefaultExtFilter($this->tipodiscapcidad);
		$sWrk = "";
		$this->BuildExtendedFilter($this->tipodiscapcidad, $sWrk, TRUE);
	...
	function CheckFilter() {
		...
		// Check tipodiscapcidad text filter
		if ($this->TextFilterApplied($this->tipodiscapcidad))
			return TRUE;
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($this->tipodiscapcidad, $sExtWrk);
	...
	function GetFilterList() {
		...
		// Field tipodiscapcidad
		$sWrk = "";
		if ($this->tipodiscapcidad->SearchValue <> "" || $this->tipodiscapcidad->SearchValue2 <> "") {
			$sWrk = "\"sv_tipodiscapcidad\":\"" . ewr_JsEncode2($this->tipodiscapcidad->SearchValue) . "\"," .
				"\"so_tipodiscapcidad\":\"" . ewr_JsEncode2($this->tipodiscapcidad->SearchOperator) . "\"," .
				"\"sc_tipodiscapcidad\":\"" . ewr_JsEncode2($this->tipodiscapcidad->SearchCondition) . "\"," .
				"\"sv2_tipodiscapcidad\":\"" . ewr_JsEncode2($this->tipodiscapcidad->SearchValue2) . "\"," .
				"\"so2_tipodiscapcidad\":\"" . ewr_JsEncode2($this->tipodiscapcidad->SearchOperator2) . "\"";
		}
		if ($sWrk == "") {
	...
	function SetupFilterList($filter) {
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "codigorude");
			$this->codigorude->SelectionList = "";
			$_SESSION["sel_viewestudiante_codigorude"] = "";
		}
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "ci");
			$this->ci->SelectionList = "";
			$_SESSION["sel_viewestudiante_ci"] = "";
		}
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "fechanacimiento");
			$this->fechanacimiento->SelectionList = "";
			$_SESSION["sel_viewestudiante_fechanacimiento"] = "";
		}
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "nrodiscapacidad");
			$this->nrodiscapacidad->SelectionList = "";
			$_SESSION["sel_viewestudiante_nrodiscapacidad"] = "";
		}
		...
		if (array_key_exists("sel_materno", $filter)) {
			...
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "materno"); // Clear extended filter
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "materno");
		...
		if (array_key_exists("sel_paterno", $filter)) {
			...
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "paterno"); // Clear extended filter
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "paterno");
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "discapacidad");
			$this->discapacidad->SelectionList = "";
			$_SESSION["sel_viewestudiante_discapacidad"] = "";
		}

		// Field tipodiscapcidad
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_tipodiscapcidad", $filter) || array_key_exists("so_tipodiscapcidad", $filter) ||
			array_key_exists("sc_tipodiscapcidad", $filter) ||
			array_key_exists("sv2_tipodiscapcidad", $filter) || array_key_exists("so2_tipodiscapcidad", $filter)) {
			$this->SetSessionFilterValues(@$filter["sv_tipodiscapcidad"], @$filter["so_tipodiscapcidad"], @$filter["sc_tipodiscapcidad"], @$filter["sv2_tipodiscapcidad"], @$filter["so2_tipodiscapcidad"], "tipodiscapcidad");
			$bRestoreFilter = TRUE;
		}
		if (array_key_exists("sel_tipodiscapcidad", $filter)) {
			...
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "tipodiscapcidad"); // Clear extended filter
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "tipodiscapcidad");
			$this->tipodiscapcidad->SelectionList = "";
			$_SESSION["sel_viewestudiante_tipodiscapcidad"] = "";
		}
		return TRUE;
	}
	...
	function GetPopupFilter() {
		...
		if (!$this->ExtendedFilterExist($this->materno)) {
			if (is_array($this->materno->SelectionList)) {
		...
		if (!$this->ExtendedFilterExist($this->paterno)) {
			if (is_array($this->paterno->SelectionList)) {
		...
		if (!$this->ExtendedFilterExist($this->tipodiscapcidad)) {
			if (is_array($this->tipodiscapcidad->SelectionList)) {
...
<input type="text" data-table="viewestudiante" data-field="x_codigorude" id="sv_codigorude" name="sv_codigorude" size="30" maxlength="100" placeholder="<?php echo $Page->codigorude->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->codigorude->SearchValue) ?>"<?php echo $Page->codigorude->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewestudiante" data-field="x_codigorude_es" id="sv_codigorude_es" name="sv_codigorude_es" size="30" maxlength="100" placeholder="<?php echo $Page->codigorude_es->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->codigorude_es->SearchValue) ?>"<?php echo $Page->codigorude_es->EditAttributes() ?>>
</span>
</div>
</div>
<div id="r_3" class="ewRow">
<div id="c_ci" class="ewCell form-group">
	<label class="ewSearchCaption ewLabel"><?php echo $Page->ci->FldCaption() ?></label>
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_ci" id="so_ci" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php
	$wrkonchange = trim(" " . @$Page->ci->EditAttrs["onchange"]);
	if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ewr_JsEncode2($wrkonchange) . "\"";
	$Page->ci->EditAttrs["onchange"] = "";
	ewr_PrependClass($Page->ci->EditAttrs["class"], "form-control");
?>
<span id="as_sv_ci" style="white-space: nowrap; z-index: 8970">
	<input type="text" id="sx_sv_ci" name="sx_sv_ci" size="30" maxlength="15" placeholder="<?php echo $Page->ci->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->ci->SearchValue) ?>"<?php echo $Page->ci->EditAttributes() ?>>
	<span id="sc_sv_ci" style="display: inline; z-index: 8970"></span>
</span>
<input type="hidden" data-table="viewestudiante" data-field="x_ci" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->ci->DisplayValueSeparator) ? json_encode($Page->ci->DisplayValueSeparator) : $Page->ci->DisplayValueSeparator) ?>" id="sv_ci" name="sv_ci" value="<?php echo ewr_HtmlEncode($Page->ci->SearchValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fviewestudianterpt.CreateAutoSuggest({"id":"sv_ci","forceSelect":false});
</script>
<button type="button" title="<?php echo ewr_HtmlEncode(str_replace("%s", ewr_RemoveHtml($Page->ci->FldCaption()), $ReportLanguage->Phrase("LookupLink", TRUE))) ?>" onclick="ewr_ModalLookupShow({lnk:this,el:'sv_ci',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" name="s_sv_ci" id="s_sv_ci" value="<?php echo $Page->ci->LookupFilterQuery(FALSE) ?>">
</span>
</div>
</div>
...
<input type="text" data-table="viewestudiante" data-field="x_fechanacimiento" id="sv_fechanacimiento" name="sv_fechanacimiento" placeholder="<?php echo $Page->fechanacimiento->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->fechanacimiento->SearchValue) ?>"<?php echo $Page->fechanacimiento->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewestudiante" data-field="x_nrodiscapacidad" id="sv_nrodiscapacidad" name="sv_nrodiscapacidad" size="30" maxlength="15" placeholder="<?php echo $Page->nrodiscapacidad->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->nrodiscapacidad->SearchValue) ?>"<?php echo $Page->nrodiscapacidad->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewestudiante" data-field="x_edad" id="sv_edad" name="sv_edad" size="30" placeholder="<?php echo $Page->edad->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->edad->SearchValue) ?>"<?php echo $Page->edad->EditAttributes() ?>>
</span>
</div>
</div>
<div id="r_17" class="ewRow">
...
<input type="text" data-table="viewestudiante" data-field="x_discapacidad" id="sv_discapacidad" name="sv_discapacidad" size="30" maxlength="100" placeholder="<?php echo $Page->discapacidad->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->discapacidad->SearchValue) ?>"<?php echo $Page->discapacidad->EditAttributes() ?>>
</span>
</div>
</div>
<div id="r_18" class="ewRow">
<div id="c_tipodiscapcidad" class="ewCell form-group">
	...
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_tipodiscapcidad" id="so_tipodiscapcidad" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->tipodiscapcidad->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="viewestudiante" data-field="x_tipodiscapcidad" id="sv_tipodiscapcidad" name="sv_tipodiscapcidad" size="30" maxlength="100" placeholder="<?php echo $Page->tipodiscapcidad->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tipodiscapcidad->SearchValue) ?>"<?php echo $Page->tipodiscapcidad->EditAttributes() ?>>
</span>
</div>
</div>
