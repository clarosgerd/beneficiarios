...
//
class crusuario extends crTableBase {
	...
	function __construct() {
		...
		// nombre
		$this->nombre = new crField('usuario', 'usuario', 'x_nombre', 'nombre', '`nombre`', 200, EWR_DATATYPE_STRING, -1);
		...
		// apellido
		$this->apellido = new crField('usuario', 'usuario', 'x_apellido', 'apellido', '`apellido`', 200, EWR_DATATYPE_STRING, -1);
		...
		// id_centro
		$this->id_centro = new crField('usuario', 'usuario', 'x_id_centro', 'id_centro', '`id_centro`', 3, EWR_DATATYPE_NUMBER, -1);
		...
		// login
		$this->login = new crField('usuario', 'usuario', 'x_login', 'login', '`login`', 200, EWR_DATATYPE_STRING, -1);
		$this->login->Sortable = TRUE; // Allow sort
		$this->login->DateFilter = "";
		$this->login->SqlSelect = "";
		$this->login->SqlOrderBy = "";
		$this->fields['login'] = &$this->login;

		// password
		$this->password = new crField('usuario', 'usuario', 'x_password', 'password', '`password`', 200, EWR_DATATYPE_STRING, -1);
		...
		// ci
		$this->ci = new crField('usuario', 'usuario', 'x_ci', 'ci', '`ci`', 200, EWR_DATATYPE_STRING, -1);
		...
		// id_rol
		$this->id_rol = new crField('usuario', 'usuario', 'x_id_rol', 'id_rol', '`id_rol`', 3, EWR_DATATYPE_NUMBER, -1);
		$this->id_rol->Sortable = TRUE; // Allow sort
		$this->id_rol->FldDefaultErrMsg = $ReportLanguage->Phrase("IncorrectInteger");
		$this->id_rol->DateFilter = "";
		$this->id_rol->SqlSelect = "";
		$this->id_rol->SqlOrderBy = "";
	...
	function GetUserIDFilter() {
		...
		if (!$Security->IsAdmin()) {
			...
			if ($sUserIDFilter <> "")
				$sUserIDFilter = '`login` IN (' . $sUserIDFilter . ')';
	...
	function ChildUserIDList($sUserID) {
		...
		// Get all values
		if ($sUserID == "-1") {
			$sSql = "SELECT `login` FROM `usuario`";
		} else {
			$sSql = "SELECT `login` FROM `usuario` WHERE `nombre` = " . ewr_QuotedValue($sUserID, EWR_DATATYPE_STRING, $this->DBID) . " OR `login` = " . ewr_QuotedValue($sUserID, EWR_DATATYPE_STRING, $this->DBID);
		}
		$rs = $conn->Execute($sSql);
		$arUser = array();
		if ($rs) {
			while (!$rs->EOF) {
				$arUser[] = $rs->fields('login');
				$rs->MoveNext();
			}
			$rs->Close();
		}
		...
		if (EWR_USER_ID_IS_HIERARCHICAL) {
			...
			while ($sUserIDList <> $sCurUserIDList) {
				$arUserWrk = array();
				$sSql = "SELECT `login` FROM `usuario` WHERE `nombre` IN (" . $sCurUserIDList . ") OR `login` = " . ewr_QuotedValue($sUserID, EWR_DATATYPE_STRING, $this->DBID);
				if ($rs = $conn->Execute($sSql)) {
					while (!$rs->EOF) {
						$arUserWrk[] = $rs->fields('login');
						$rs->MoveNext();
					}
					$rs->Close();
				}
	...
	function UserIDList($ar) {
		...
		if (is_array($ar)) {
			...
			for ($i = 0; $i < $cntar; $i++) {
				if ($sWrk <> "")
					$sWrk .= ", ";
				$sWrk .= ewr_QuotedValue($ar[$i], EWR_DATATYPE_STRING, $this->DBID);
			}
		}
		return $sWrk;
	}
