...
$otrasorganizaciones_add = NULL; // Initialize page object first

class cotrasorganizaciones_add extends cotrasorganizaciones {
	...
	// Page object name
	var $PageObjName = 'otrasorganizaciones_add';
	...
	function Page_Init() {
		...
		if (!$Security->CanAdd()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("otrasorganizacioneslist.php"));
			else
		...
		if ($url <> "") {
			...
			if ($this->IsModal) { // Show as modal
				...
				if ($pageName != $this->GetListUrl()) { // Not List page
					...
					if ($pageName == "otrasorganizacionesview.php")
						$row["view"] = "1";
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("otrasorganizacioneslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "otrasorganizacioneslist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to List page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "otrasorganizacionesview.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("otrasorganizacioneslist.php"), "", $this->TableVar, TRUE);
...
// Create page object
if (!isset($otrasorganizaciones_add)) $otrasorganizaciones_add = new cotrasorganizaciones_add();

// Page init
$otrasorganizaciones_add->Page_Init();

// Page main
$otrasorganizaciones_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$otrasorganizaciones_add->Page_Render();
?>
...
var CurrentForm = fotrasorganizacionesadd = new ew_Form("fotrasorganizacionesadd", "add");

// Validate form
fotrasorganizacionesadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $otrasorganizaciones->id->FldCaption(), $otrasorganizaciones->id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_id");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($otrasorganizaciones->id->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_nombre");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $otrasorganizaciones->nombre->FldCaption(), $otrasorganizaciones->nombre->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_departamento");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $otrasorganizaciones->departamento->FldCaption(), $otrasorganizaciones->departamento->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_ciudad");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $otrasorganizaciones->ciudad->FldCaption(), $otrasorganizaciones->ciudad->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_direccion");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $otrasorganizaciones->direccion->FldCaption(), $otrasorganizaciones->direccion->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_telefono");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $otrasorganizaciones->telefono->FldCaption(), $otrasorganizaciones->telefono->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "__email");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $otrasorganizaciones->_email->FldCaption(), $otrasorganizaciones->_email->ReqErrMsg)) ?>");
...
fotrasorganizacionesadd.Form_CustomValidate = 
...
fotrasorganizacionesadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fotrasorganizacionesadd.Lists["x_ciudad"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fotrasorganizacionesadd.Lists["x_ciudad"].Options = <?php echo json_encode($otrasorganizaciones_add->ciudad->Options()) ?>;
fotrasorganizacionesadd.Lists["x_id_persona"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","x_apellidopaterno","x_apellidomaterno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"persona"};
fotrasorganizacionesadd.Lists["x_id_persona"].Data = "<?php echo $otrasorganizaciones_add->id_persona->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $otrasorganizaciones_add->ShowPageHeader(); ?>
<?php
$otrasorganizaciones_add->ShowMessage();
?>
<form name="fotrasorganizacionesadd" id="fotrasorganizacionesadd" class="<?php echo $otrasorganizaciones_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($otrasorganizaciones_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $otrasorganizaciones_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="otrasorganizaciones">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($otrasorganizaciones_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($otrasorganizaciones->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_otrasorganizaciones_id" for="x_id" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->id->CellAttributes() ?>>
<span id="el_otrasorganizaciones_id">
<input type="text" data-table="otrasorganizaciones" data-field="x_id" name="x_id" id="x_id" size="30" placeholder="<?php echo ew_HtmlEncode($otrasorganizaciones->id->getPlaceHolder()) ?>" value="<?php echo $otrasorganizaciones->id->EditValue ?>"<?php echo $otrasorganizaciones->id->EditAttributes() ?>>
</span>
<?php echo $otrasorganizaciones->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($otrasorganizaciones->nombre->Visible) { // nombre ?>
	<div id="r_nombre" class="form-group">
		<label id="elh_otrasorganizaciones_nombre" for="x_nombre" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->nombre->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->nombre->CellAttributes() ?>>
<span id="el_otrasorganizaciones_nombre">
...
<?php echo $otrasorganizaciones->nombre->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($otrasorganizaciones->departamento->Visible) { // departamento ?>
	<div id="r_departamento" class="form-group">
		<label id="elh_otrasorganizaciones_departamento" for="x_departamento" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->departamento->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->departamento->CellAttributes() ?>>
<span id="el_otrasorganizaciones_departamento">
...
<?php echo $otrasorganizaciones->departamento->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($otrasorganizaciones->ciudad->Visible) { // ciudad ?>
	<div id="r_ciudad" class="form-group">
		<label id="elh_otrasorganizaciones_ciudad" for="x_ciudad" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->ciudad->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->ciudad->CellAttributes() ?>>
<span id="el_otrasorganizaciones_ciudad">
...
<?php echo $otrasorganizaciones->ciudad->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($otrasorganizaciones->direccion->Visible) { // direccion ?>
	<div id="r_direccion" class="form-group">
		<label id="elh_otrasorganizaciones_direccion" for="x_direccion" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->direccion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->direccion->CellAttributes() ?>>
<span id="el_otrasorganizaciones_direccion">
...
<?php echo $otrasorganizaciones->direccion->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($otrasorganizaciones->telefono->Visible) { // telefono ?>
	<div id="r_telefono" class="form-group">
		<label id="elh_otrasorganizaciones_telefono" for="x_telefono" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->telefono->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->telefono->CellAttributes() ?>>
<span id="el_otrasorganizaciones_telefono">
...
<?php echo $otrasorganizaciones->telefono->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($otrasorganizaciones->_email->Visible) { // email ?>
	<div id="r__email" class="form-group">
		<label id="elh_otrasorganizaciones__email" for="x__email" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->_email->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->_email->CellAttributes() ?>>
<span id="el_otrasorganizaciones__email">
...
<?php echo $otrasorganizaciones->_email->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($otrasorganizaciones->id_persona->Visible) { // id_persona ?>
	<div id="r_id_persona" class="form-group">
		<label id="elh_otrasorganizaciones_id_persona" for="x_id_persona" class="<?php echo $otrasorganizaciones_add->LeftColumnClass ?>"><?php echo $otrasorganizaciones->id_persona->FldCaption() ?></label>
		<div class="<?php echo $otrasorganizaciones_add->RightColumnClass ?>"><div<?php echo $otrasorganizaciones->id_persona->CellAttributes() ?>>
<span id="el_otrasorganizaciones_id_persona">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next(":not([disabled])").click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_id_persona"><?php echo (strval($otrasorganizaciones->id_persona->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $otrasorganizaciones->id_persona->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($otrasorganizaciones->id_persona->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_id_persona',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($otrasorganizaciones->id_persona->ReadOnly || $otrasorganizaciones->id_persona->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="otrasorganizaciones" data-field="x_id_persona" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $otrasorganizaciones->id_persona->DisplayValueSeparatorAttribute() ?>" name="x_id_persona" id="x_id_persona" value="<?php echo $otrasorganizaciones->id_persona->CurrentValue ?>"<?php echo $otrasorganizaciones->id_persona->EditAttributes() ?>>
<?php if (AllowAdd(CurrentProjectID() . "persona") && !$otrasorganizaciones->id_persona->ReadOnly) { ?>
<button type="button" title="<?php echo ew_HtmlTitle($Language->Phrase("AddLink")) . "&nbsp;" . $otrasorganizaciones->id_persona->FldCaption() ?>" onclick="ew_AddOptDialogShow({lnk:this,el:'x_id_persona',url:'personaaddopt.php'});" class="ewAddOptBtn btn btn-default btn-sm" id="aol_x_id_persona"><span class="glyphicon glyphicon-plus ewIcon"></span><span class="hide"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $otrasorganizaciones->id_persona->FldCaption() ?></span></button>
<?php } ?>
</span>
<?php echo $otrasorganizaciones->id_persona->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$otrasorganizaciones_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $otrasorganizaciones_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $otrasorganizaciones_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fotrasorganizacionesadd.Init();
</script>
<?php
$otrasorganizaciones_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$otrasorganizaciones_add->Page_Terminate();
?>
