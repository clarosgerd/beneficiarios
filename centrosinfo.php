...
//
// Table class for centros
//
class ccentros extends cTable {
	var $id;
	...
	function __construct() {
		...
		$this->TableVar = 'centros';
		$this->TableName = 'centros';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`centros`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('centros', 'centros', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = FALSE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// nombreinstitucion
		$this->nombreinstitucion = new cField('centros', 'centros', 'x_nombreinstitucion', 'nombreinstitucion', '`nombreinstitucion`', '`nombreinstitucion`', 200, -1, FALSE, '`nombreinstitucion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nombreinstitucion->Sortable = TRUE; // Allow sort
		$this->fields['nombreinstitucion'] = &$this->nombreinstitucion;

		// direccion
		$this->direccion = new cField('centros', 'centros', 'x_direccion', 'direccion', '`direccion`', '`direccion`', 200, -1, FALSE, '`direccion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->direccion->Sortable = TRUE; // Allow sort
		$this->fields['direccion'] = &$this->direccion;

		// telefono
		$this->telefono = new cField('centros', 'centros', 'x_telefono', 'telefono', '`telefono`', '`telefono`', 200, -1, FALSE, '`telefono`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->telefono->Sortable = TRUE; // Allow sort
		$this->fields['telefono'] = &$this->telefono;

		// ciudad
		$this->ciudad = new cField('centros', 'centros', 'x_ciudad', 'ciudad', '`ciudad`', '`ciudad`', 200, -1, FALSE, '`ciudad`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// provincia
		$this->provincia = new cField('centros', 'centros', 'x_provincia', 'provincia', '`provincia`', '`provincia`', 3, -1, FALSE, '`provincia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// municipio
		$this->municipio = new cField('centros', 'centros', 'x_municipio', 'municipio', '`municipio`', '`municipio`', 3, -1, FALSE, '`municipio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_persona
		$this->id_persona = new cField('centros', 'centros', 'x_id_persona', 'id_persona', '`id_persona`', '`id_persona`', 3, -1, FALSE, '`EV__id_persona`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`centros`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(COALESCE(`nombre`, ''),'" . ew_ValueSeparator(1, $this->id_persona) . "',COALESCE(`apellidopaterno`,''),'" . ew_ValueSeparator(2, $this->id_persona) . "',COALESCE(`apellidomaterno`,'')) FROM `persona` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `centros`.`id_persona` LIMIT 1) AS `EV__id_persona` FROM `centros`" .
			") `EW_TMP_TABLE`";
	...
	function getSqlWhere() { // Where
		...
		$this->TableFilter = "`id` like '".$_SESSION["centro"]."'";
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "centrosedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "centrosadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("centrosview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("centrosview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "centrosadd.php?" . $this->UrlParm($parm);
		else
			$url = "centrosadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("centrosedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("centrosadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("centrosdelete.php", $this->UrlParm());
	}
