...
$diagnosticoaudiologia_add = NULL; // Initialize page object first

class cdiagnosticoaudiologia_add extends cdiagnosticoaudiologia {
	...
	// Page object name
	var $PageObjName = 'diagnosticoaudiologia_add';
	...
	function Page_Init() {
		...
		if (!$Security->CanAdd()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("diagnosticoaudiologialist.php"));
			else
		...
		if ($url <> "") {
			...
			if ($this->IsModal) { // Show as modal
				...
				if ($pageName != $this->GetListUrl()) { // Not List page
					...
					if ($pageName == "diagnosticoaudiologiaview.php")
						$row["view"] = "1";
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("diagnosticoaudiologialist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "diagnosticoaudiologialist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to List page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "diagnosticoaudiologiaview.php")
		...
		$this->resultado->ViewCustomAttributes = "";
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				...
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					...
					$this->id_audiologia->EditValue = $this->id_audiologia->DisplayValue($arwrk);
					$rswrk->Close();
				} else {
					$this->id_audiologia->EditValue = ew_HtmlEncode($this->id_audiologia->CurrentValue);
				}
			} else {
				$this->id_audiologia->EditValue = NULL;
			}
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("diagnosticoaudiologialist.php"), "", $this->TableVar, TRUE);
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		switch ($fld->FldVar) {
		case "x_id_audiologia":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `observaciones` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `audiologia`";
			$sWhereWrk = "{filter}";
	...
	function SetupAutoSuggestFilters($fld, $pageId = null) {
		...
		case "x_id_audiologia":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id`, `observaciones` AS `DispFld` FROM `audiologia`";
			$sWhereWrk = "`observaciones` LIKE '{query_value}%'";
			$fld->LookupFilters = array();
...
// Create page object
if (!isset($diagnosticoaudiologia_add)) $diagnosticoaudiologia_add = new cdiagnosticoaudiologia_add();

// Page init
$diagnosticoaudiologia_add->Page_Init();

// Page main
$diagnosticoaudiologia_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$diagnosticoaudiologia_add->Page_Render();
?>
...
var CurrentForm = fdiagnosticoaudiologiaadd = new ew_Form("fdiagnosticoaudiologiaadd", "add");

// Validate form
fdiagnosticoaudiologiaadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $diagnosticoaudiologia->id_audiologia->FldCaption(), $diagnosticoaudiologia->id_audiologia->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_id_audiologia");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($diagnosticoaudiologia->id_audiologia->FldErrMsg()) ?>");
...
fdiagnosticoaudiologiaadd.Form_CustomValidate = 
...
fdiagnosticoaudiologiaadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fdiagnosticoaudiologiaadd.Lists["x_id_audiologia"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_observaciones","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"audiologia"};
fdiagnosticoaudiologiaadd.Lists["x_id_audiologia"].Data = "<?php echo $diagnosticoaudiologia_add->id_audiologia->LookupFilterQuery(FALSE, "add") ?>";
fdiagnosticoaudiologiaadd.AutoSuggests["x_id_audiologia"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $diagnosticoaudiologia_add->id_audiologia->LookupFilterQuery(TRUE, "add"))) ?>;
fdiagnosticoaudiologiaadd.Lists["x_id_tipodiagnosticoaudiologia"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipodiagnosticoaudiologia"};
fdiagnosticoaudiologiaadd.Lists["x_id_tipodiagnosticoaudiologia"].Data = "<?php echo $diagnosticoaudiologia_add->id_tipodiagnosticoaudiologia->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $diagnosticoaudiologia_add->ShowPageHeader(); ?>
<?php
$diagnosticoaudiologia_add->ShowMessage();
?>
<form name="fdiagnosticoaudiologiaadd" id="fdiagnosticoaudiologiaadd" class="<?php echo $diagnosticoaudiologia_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($diagnosticoaudiologia_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $diagnosticoaudiologia_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="diagnosticoaudiologia">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($diagnosticoaudiologia_add->IsModal) ?>">
<?php if ($diagnosticoaudiologia->getCurrentMasterTable() == "audiologia") { ?>
...
<?php if ($diagnosticoaudiologia->id_audiologia->Visible) { // id_audiologia ?>
	<div id="r_id_audiologia" class="form-group">
		<label id="elh_diagnosticoaudiologia_id_audiologia" class="<?php echo $diagnosticoaudiologia_add->LeftColumnClass ?>"><?php echo $diagnosticoaudiologia->id_audiologia->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $diagnosticoaudiologia_add->RightColumnClass ?>"><div<?php echo $diagnosticoaudiologia->id_audiologia->CellAttributes() ?>>
<?php if ($diagnosticoaudiologia->id_audiologia->getSessionValue() <> "") { ?>
<span id="el_diagnosticoaudiologia_id_audiologia">
<span<?php echo $diagnosticoaudiologia->id_audiologia->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $diagnosticoaudiologia->id_audiologia->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_id_audiologia" name="x_id_audiologia" value="<?php echo ew_HtmlEncode($diagnosticoaudiologia->id_audiologia->CurrentValue) ?>">
<?php } else { ?>
<span id="el_diagnosticoaudiologia_id_audiologia">
<?php
$wrkonchange = trim(" " . @$diagnosticoaudiologia->id_audiologia->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$diagnosticoaudiologia->id_audiologia->EditAttrs["onchange"] = "";
?>
<span id="as_x_id_audiologia" style="white-space: nowrap; z-index: 8980">
	<input type="text" name="sv_x_id_audiologia" id="sv_x_id_audiologia" value="<?php echo $diagnosticoaudiologia->id_audiologia->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($diagnosticoaudiologia->id_audiologia->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($diagnosticoaudiologia->id_audiologia->getPlaceHolder()) ?>"<?php echo $diagnosticoaudiologia->id_audiologia->EditAttributes() ?>>
</span>
<input type="hidden" data-table="diagnosticoaudiologia" data-field="x_id_audiologia" data-value-separator="<?php echo $diagnosticoaudiologia->id_audiologia->DisplayValueSeparatorAttribute() ?>" name="x_id_audiologia" id="x_id_audiologia" value="<?php echo ew_HtmlEncode($diagnosticoaudiologia->id_audiologia->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fdiagnosticoaudiologiaadd.CreateAutoSuggest({"id":"x_id_audiologia","forceSelect":false});
</script>
</span>
<?php } ?>
<?php echo $diagnosticoaudiologia->id_audiologia->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($diagnosticoaudiologia->id_tipodiagnosticoaudiologia->Visible) { // id_tipodiagnosticoaudiologia ?>
	<div id="r_id_tipodiagnosticoaudiologia" class="form-group">
		<label id="elh_diagnosticoaudiologia_id_tipodiagnosticoaudiologia" for="x_id_tipodiagnosticoaudiologia" class="<?php echo $diagnosticoaudiologia_add->LeftColumnClass ?>"><?php echo $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->FldCaption() ?></label>
		<div class="<?php echo $diagnosticoaudiologia_add->RightColumnClass ?>"><div<?php echo $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->CellAttributes() ?>>
<span id="el_diagnosticoaudiologia_id_tipodiagnosticoaudiologia">
<select data-table="diagnosticoaudiologia" data-field="x_id_tipodiagnosticoaudiologia" data-value-separator="<?php echo $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->DisplayValueSeparatorAttribute() ?>" id="x_id_tipodiagnosticoaudiologia" name="x_id_tipodiagnosticoaudiologia"<?php echo $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->EditAttributes() ?>>
<?php echo $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->SelectOptionListHtml("x_id_tipodiagnosticoaudiologia") ?>
</select>
<?php if (AllowAdd(CurrentProjectID() . "tipodiagnosticoaudiologia") && !$diagnosticoaudiologia->id_tipodiagnosticoaudiologia->ReadOnly) { ?>
<button type="button" title="<?php echo ew_HtmlTitle($Language->Phrase("AddLink")) . "&nbsp;" . $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->FldCaption() ?>" onclick="ew_AddOptDialogShow({lnk:this,el:'x_id_tipodiagnosticoaudiologia',url:'tipodiagnosticoaudiologiaaddopt.php'});" class="ewAddOptBtn btn btn-default btn-sm" id="aol_x_id_tipodiagnosticoaudiologia"><span class="glyphicon glyphicon-plus ewIcon"></span><span class="hide"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->FldCaption() ?></span></button>
<?php } ?>
</span>
<?php echo $diagnosticoaudiologia->id_tipodiagnosticoaudiologia->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($diagnosticoaudiologia->resultado->Visible) { // resultado ?>
	<div id="r_resultado" class="form-group">
		<label id="elh_diagnosticoaudiologia_resultado" for="x_resultado" class="<?php echo $diagnosticoaudiologia_add->LeftColumnClass ?>"><?php echo $diagnosticoaudiologia->resultado->FldCaption() ?></label>
		<div class="<?php echo $diagnosticoaudiologia_add->RightColumnClass ?>"><div<?php echo $diagnosticoaudiologia->resultado->CellAttributes() ?>>
<span id="el_diagnosticoaudiologia_resultado">
<input type="text" data-table="diagnosticoaudiologia" data-field="x_resultado" name="x_resultado" id="x_resultado" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($diagnosticoaudiologia->resultado->getPlaceHolder()) ?>" value="<?php echo $diagnosticoaudiologia->resultado->EditValue ?>"<?php echo $diagnosticoaudiologia->resultado->EditAttributes() ?>>
</span>
<?php echo $diagnosticoaudiologia->resultado->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$diagnosticoaudiologia_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $diagnosticoaudiologia_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $diagnosticoaudiologia_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fdiagnosticoaudiologiaadd.Init();
</script>
<?php
$diagnosticoaudiologia_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$diagnosticoaudiologia_add->Page_Terminate();
?>
