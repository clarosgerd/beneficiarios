...
function CurrentProjectID() {
	...
	return "{707530BA-BEB7-415A-B683-2C9753B31FA3}";
}
...
class crAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE, $provider = "") {
		...
		if (!$valid) {
			if (EWR_CASE_SENSITIVE_PASSWORD) {
				$valid = (!$customValid && EWR_ADMIN_USER_NAME == $usr && EWR_ADMIN_PASSWORD == $pwd) ||
					($customValid && EWR_ADMIN_USER_NAME == $usr);
			} else {
				$valid = (!$customValid && ewr_SameText(EWR_ADMIN_USER_NAME, $usr) && ewr_SameText(EWR_ADMIN_PASSWORD, $pwd)) ||
					($customValid && ewr_SameText(EWR_ADMIN_USER_NAME, $usr));
			}
			if ($valid) {
				$_SESSION[EWR_SESSION_STATUS] = "login";
				$_SESSION[EWR_SESSION_SYSTEM_ADMIN] = 1; // System Administrator
				$this->setCurrentUserName($ReportLanguage->Phrase("UserAdministrator")); // Load user name
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ewr_ComparePassword($rs->fields('password'), $pwd, $encrypted);
					if ($valid) {
						$_SESSION[EWR_SESSION_STATUS] = "login";
						$_SESSION[EWR_SESSION_SYSTEM_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('login')); // Load user name
						$this->setSessionUserID($rs->fields('login')); // Load User ID
						$this->setSessionParentUserID($rs->fields('nombre')); // Load parent User ID
						if (is_null($rs->fields('id_rol'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('id_rol'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			$sParentFilter = '`nombre` IN (' . $this->UserIDList() . ')';
			$sFilter = "($sFilter) OR ($sParentFilter)";
			$sSql = EWR_LOGIN_SELECT_SQL . " WHERE " . $sFilter;
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('login'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
			...
			if (EWR_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`nombre` IN (' . $sCurUserIDList . ')';
					$sSql = EWR_LOGIN_SELECT_SQL . " WHERE " . $sFilter;
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('login'));
	...
	function UserIDList() {
		...
		$len = count($ar);
		for ($i = 0; $i < $len; $i++)
			$ar[$i] = ewr_QuotedValue($ar[$i], EWR_DATATYPE_STRING, EWR_USER_TABLE_DBID);
		return implode(", ", $ar);
	}
	...
	function ParentUserIDList($userid) {
		...
		if (trim(strval($userid)) == strval(CurrentUserID())) {
			if (strval(CurrentParentUserID()) <> "")
				$result = ewr_QuotedValue(CurrentParentUserID(), EWR_DATATYPE_STRING, EWR_USER_TABLE_DBID);
			return $result;
		}

		// One level only, must be CurrentUserID
		if (!EWR_USER_ID_IS_HIERARCHICAL) {
			return ewr_QuotedValue(CurrentUserID(), EWR_DATATYPE_STRING, EWR_USER_TABLE_DBID);
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) <> trim(strval($userid))) {
					if ($result <> "")
						$result .= ", ";
					$result .= ewr_QuotedValue($ar[$i], EWR_DATATYPE_STRING, EWR_USER_TABLE_DBID);
				}
			}
			return $result;
		}
