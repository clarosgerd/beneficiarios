...
$atencionescolaraudiologia = NULL;

//
// Table class for atencionescolaraudiologia
//
class catencionescolaraudiologia extends cTable {
	var $id;
	var $id_escolar;
	var $id_especialista;
	var $id_referencia;
	...
	function __construct() {
		...
		$this->TableVar = 'atencionescolaraudiologia';
		$this->TableName = 'atencionescolaraudiologia';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`atencionescolaraudiologia`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('atencionescolaraudiologia', 'atencionescolaraudiologia', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// id_escolar
		$this->id_escolar = new cField('atencionescolaraudiologia', 'atencionescolaraudiologia', 'x_id_escolar', 'id_escolar', '`id_escolar`', '`id_escolar`', 3, -1, FALSE, '`EV__id_escolar`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_especialista
		$this->id_especialista = new cField('atencionescolaraudiologia', 'atencionescolaraudiologia', 'x_id_especialista', 'id_especialista', '`id_especialista`', '`id_especialista`', 3, -1, FALSE, '`EV__id_especialista`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_referencia
		$this->id_referencia = new cField('atencionescolaraudiologia', 'atencionescolaraudiologia', 'x_id_referencia', 'id_referencia', '`id_referencia`', '`id_referencia`', 3, -1, FALSE, '`id_referencia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`atencionescolaraudiologia`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`nombres`,'" . ew_ValueSeparator(1, $this->id_escolar) . "',`apellidopaterno`,'" . ew_ValueSeparator(2, $this->id_escolar) . "',`apellidomaterno`) FROM `escolar` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `atencionescolaraudiologia`.`id_escolar` LIMIT 1) AS `EV__id_escolar`, (SELECT CONCAT(`nombres`,'" . ew_ValueSeparator(1, $this->id_especialista) . "',`apellidopaterno`,'" . ew_ValueSeparator(2, $this->id_especialista) . "',`apellidomaterno`,'" . ew_ValueSeparator(3, $this->id_especialista) . "',`nombreespecialidad`) FROM `especialista` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `atencionescolaraudiologia`.`id_especialista` LIMIT 1) AS `EV__id_especialista` FROM `atencionescolaraudiologia`" .
			") `EW_TMP_TABLE`";
	...
	function UseVirtualFields() {
		...
		if ($this->id_escolar->AdvancedSearch->SearchValue <> "" ||
			$this->id_escolar->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->id_escolar->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->id_escolar->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "atencionescolaraudiologialist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "atencionescolaraudiologiaedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "atencionescolaraudiologiaadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("atencionescolaraudiologiaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("atencionescolaraudiologiaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "atencionescolaraudiologiaadd.php?" . $this->UrlParm($parm);
		else
			$url = "atencionescolaraudiologiaadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("atencionescolaraudiologiaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("atencionescolaraudiologiaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("atencionescolaraudiologiadelete.php", $this->UrlParm());
	}
	...
	$especialidad = ew_ExecuteScalar("SELECT especialidad FROM especialista WHERE 	id = '".$rsnew["id_especialista"]."'");
		...
		$MyResult = ew_Execute("INSERT INTO audiologia (id_especialista,fecha,id_escolar,id_atencion,especialidad,id_centro) VALUES ('".$rsnew["id_especialista"]."',NOW(),'".$rsnew["id_escolar"]."','".$rsnew["id"]."','".$tipoespecialidad."','".$_SESSION["centro"]."')");
	}
