...
<?php include_once "diagnosticoaudiologiarptinfo.php" ?>
<?php
...
$diagnosticoaudiologia_rpt = NULL; // Initialize page object first

class crdiagnosticoaudiologia_rpt extends crdiagnosticoaudiologia {
	...
	// Page object name
	var $PageObjName = 'diagnosticoaudiologia_rpt';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["diagnosticoaudiologia"])) {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'diagnosticoaudiologia', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fdiagnosticoaudiologiarpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'diagnosticoaudiologia');
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_diagnosticoaudiologia\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_diagnosticoaudiologia',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fdiagnosticoaudiologiarpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fdiagnosticoaudiologiarpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fdiagnosticoaudiologiarpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['id_tipodiagnosticoaudiologia'] = ewr_Conv($rs->fields('id_tipodiagnosticoaudiologia'), 3);
		...
		if (!$rs->EOF) {
			...
			$this->Val[3] = $this->id_tipodiagnosticoaudiologia->CurrentValue;
			$this->Val[4] = $this->resultado->CurrentValue;
		} else {
			$this->id->setDbValue("");
			$this->id_audiologia->setDbValue("");
			$this->id_tipodiagnosticoaudiologia->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->id_tipodiagnosticoaudiologia->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// id_tipodiagnosticoaudiologia
			$CurrentValue = $this->id_tipodiagnosticoaudiologia->CurrentValue;
			$ViewValue = &$this->id_tipodiagnosticoaudiologia->ViewValue;
			$ViewAttrs = &$this->id_tipodiagnosticoaudiologia->ViewAttrs;
			$CellAttrs = &$this->id_tipodiagnosticoaudiologia->CellAttrs;
			$HrefValue = &$this->id_tipodiagnosticoaudiologia->HrefValue;
			$LinkAttrs = &$this->id_tipodiagnosticoaudiologia->LinkAttrs;
			$this->Cell_Rendered($this->id_tipodiagnosticoaudiologia, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->id_tipodiagnosticoaudiologia->Visible) $this->DtlColumnCount += 1;
...
// Create page object
if (!isset($diagnosticoaudiologia_rpt)) $diagnosticoaudiologia_rpt = new crdiagnosticoaudiologia_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$diagnosticoaudiologia_rpt;
...
// Create page object
var diagnosticoaudiologia_rpt = new ewr_Page("diagnosticoaudiologia_rpt");

// Page properties
diagnosticoaudiologia_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = diagnosticoaudiologia_rpt.PageID;
</script>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_diagnosticoaudiologia" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id"><div class="diagnosticoaudiologia_id"><span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id">
<?php if ($Page->SortUrl($Page->id) == "") { ?>
		<div class="ewTableHeaderBtn diagnosticoaudiologia_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer diagnosticoaudiologia_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id_audiologia"><div class="diagnosticoaudiologia_id_audiologia"><span class="ewTableHeaderCaption"><?php echo $Page->id_audiologia->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id_audiologia">
<?php if ($Page->SortUrl($Page->id_audiologia) == "") { ?>
		<div class="ewTableHeaderBtn diagnosticoaudiologia_id_audiologia">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_audiologia->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer diagnosticoaudiologia_id_audiologia" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id_audiologia) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_audiologia->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->id_tipodiagnosticoaudiologia->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id_tipodiagnosticoaudiologia"><div class="diagnosticoaudiologia_id_tipodiagnosticoaudiologia"><span class="ewTableHeaderCaption"><?php echo $Page->id_tipodiagnosticoaudiologia->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id_tipodiagnosticoaudiologia">
<?php if ($Page->SortUrl($Page->id_tipodiagnosticoaudiologia) == "") { ?>
		<div class="ewTableHeaderBtn diagnosticoaudiologia_id_tipodiagnosticoaudiologia">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_tipodiagnosticoaudiologia->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer diagnosticoaudiologia_id_tipodiagnosticoaudiologia" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id_tipodiagnosticoaudiologia) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_tipodiagnosticoaudiologia->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->id_tipodiagnosticoaudiologia->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->id_tipodiagnosticoaudiologia->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="resultado"><div class="diagnosticoaudiologia_resultado"><span class="ewTableHeaderCaption"><?php echo $Page->resultado->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="resultado">
<?php if ($Page->SortUrl($Page->resultado) == "") { ?>
		<div class="ewTableHeaderBtn diagnosticoaudiologia_resultado">
			<span class="ewTableHeaderCaption"><?php echo $Page->resultado->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer diagnosticoaudiologia_resultado" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->resultado) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->resultado->FldCaption() ?></span>
...
<?php } ?>
<?php if ($Page->id_tipodiagnosticoaudiologia->Visible) { ?>
		<td data-field="id_tipodiagnosticoaudiologia"<?php echo $Page->id_tipodiagnosticoaudiologia->CellAttributes() ?>>
<span<?php echo $Page->id_tipodiagnosticoaudiologia->ViewAttributes() ?>><?php echo $Page->id_tipodiagnosticoaudiologia->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->resultado->Visible) { ?>
...
<?php include "diagnosticoaudiologiarptpager.php" ?>
