...
$atencionotrosaudiologia = NULL;

//
// Table class for atencionotrosaudiologia
//
class catencionotrosaudiologia extends cTable {
	var $id;
	var $id_otros;
	var $id_especialista;
	var $id_referencia;
	...
	function __construct() {
		...
		$this->TableVar = 'atencionotrosaudiologia';
		$this->TableName = 'atencionotrosaudiologia';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`atencionotrosaudiologia`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('atencionotrosaudiologia', 'atencionotrosaudiologia', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// id_otros
		$this->id_otros = new cField('atencionotrosaudiologia', 'atencionotrosaudiologia', 'x_id_otros', 'id_otros', '`id_otros`', '`id_otros`', 3, -1, FALSE, '`EV__id_otros`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_especialista
		$this->id_especialista = new cField('atencionotrosaudiologia', 'atencionotrosaudiologia', 'x_id_especialista', 'id_especialista', '`id_especialista`', '`id_especialista`', 3, -1, FALSE, '`EV__id_especialista`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_referencia
		$this->id_referencia = new cField('atencionotrosaudiologia', 'atencionotrosaudiologia', 'x_id_referencia', 'id_referencia', '`id_referencia`', '`id_referencia`', 3, -1, FALSE, '`id_referencia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`atencionotrosaudiologia`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(`nombre`,'" . ew_ValueSeparator(1, $this->id_otros) . "',`apellidopaterno`,'" . ew_ValueSeparator(2, $this->id_otros) . "',`apellidomaterno`) FROM `otros` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `atencionotrosaudiologia`.`id_otros` LIMIT 1) AS `EV__id_otros`, (SELECT CONCAT(`nombres`,'" . ew_ValueSeparator(1, $this->id_especialista) . "',`apellidopaterno`,'" . ew_ValueSeparator(2, $this->id_especialista) . "',`apellidomaterno`,'" . ew_ValueSeparator(3, $this->id_especialista) . "',`nombreespecialidad`) FROM `especialista` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `atencionotrosaudiologia`.`id_especialista` LIMIT 1) AS `EV__id_especialista` FROM `atencionotrosaudiologia`" .
			") `EW_TMP_TABLE`";
	...
	function UseVirtualFields() {
		...
		if ($this->id_otros->AdvancedSearch->SearchValue <> "" ||
			$this->id_otros->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->id_otros->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->id_otros->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "atencionotrosaudiologialist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "atencionotrosaudiologiaedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "atencionotrosaudiologiaadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("atencionotrosaudiologiaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("atencionotrosaudiologiaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "atencionotrosaudiologiaadd.php?" . $this->UrlParm($parm);
		else
			$url = "atencionotrosaudiologiaadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("atencionotrosaudiologiaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("atencionotrosaudiologiaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("atencionotrosaudiologiadelete.php", $this->UrlParm());
	}
	...
	//echo "Row Inserted"
		...
		$tipoespecialidad = ew_ExecuteScalar("SELECT nombre FROM tipoespecialidad WHERE 	id = '".$especialidad."'");
			$MyResult = ew_Execute("INSERT INTO audiologia (id_especialista,fecha,id_atencion,id_otros,especialidad,id_centro) VALUES ('".$rsnew["id_especialista"]."',NOW(),'".$rsnew["id"]."','".$rsnew["id_otros"]."','".$tipoespecialidad."','".$_SESSION["centro"]."')");
