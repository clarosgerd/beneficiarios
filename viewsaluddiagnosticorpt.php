...
<?php include_once "viewsaluddiagnosticorptinfo.php" ?>
<?php
...
$viewsaluddiagnostico_rpt = NULL; // Initialize page object first

class crviewsaluddiagnostico_rpt extends crviewsaluddiagnostico {
	...
	// Page object name
	var $PageObjName = 'viewsaluddiagnostico_rpt';
	...
	function __construct() {
		...
		// Table object (viewsaluddiagnostico)
		if (!isset($GLOBALS["viewsaluddiagnostico"])) {
			$GLOBALS["viewsaluddiagnostico"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["viewsaluddiagnostico"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'viewsaluddiagnostico', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fviewsaluddiagnosticorpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'viewsaluddiagnostico');
		...
		$this->diagnostico->PlaceHolder = $this->diagnostico->FldCaption();
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_viewsaluddiagnostico\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_viewsaluddiagnostico',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fviewsaluddiagnosticorpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fviewsaluddiagnosticorpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fviewsaluddiagnosticorpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->diagnostico->SelectionList = "";
		$this->diagnostico->DefaultSelectionList = "";
		$this->diagnostico->ValueList = "";
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['diagnostico'] = ewr_Conv($rs->fields('diagnostico'), 200);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'viewsaluddiagnostico_especialidad') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'viewsaluddiagnostico_resultado') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			// Build distinct values for diagnostico
			if ($popupname == 'viewsaluddiagnostico_diagnostico') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;

				// Call Page Filtering event
				$this->Page_Filtering($this->diagnostico, $sFilter, "popup");
				$sSql = ewr_BuildReportSql($this->diagnostico->SqlSelect, $this->getSqlWhere(), $this->getSqlGroupBy(), $this->getSqlHaving(), $this->diagnostico->SqlOrderBy, $sFilter, "");
				$rswrk = $conn->Execute($sSql);
				while ($rswrk && !$rswrk->EOF) {
					$this->diagnostico->setDbValue($rswrk->fields[0]);
					$this->diagnostico->ViewValue = @$rswrk->fields[1];
					if (is_null($this->diagnostico->CurrentValue)) {
						$bNullValue = TRUE;
					} elseif ($this->diagnostico->CurrentValue == "") {
						$bEmptyValue = TRUE;
					} else {
						ewr_SetupDistinctValues($this->diagnostico->ValueList, $this->diagnostico->CurrentValue, $this->diagnostico->ViewValue, FALSE, $this->diagnostico->FldDelimiter);
					}
					$rswrk->MoveNext();
				}
				if ($rswrk)
					$rswrk->Close();
				if ($bEmptyValue)
					ewr_SetupDistinctValues($this->diagnostico->ValueList, EWR_EMPTY_VALUE, $ReportLanguage->Phrase("EmptyLabel"), FALSE);
				if ($bNullValue)
					ewr_SetupDistinctValues($this->diagnostico->ValueList, EWR_NULL_VALUE, $ReportLanguage->Phrase("NullLabel"), FALSE);
				$fld = &$this->diagnostico;
			}
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				...
				$this->ClearSessionSelection('diagnostico');
				$this->ResetPager();
			}
		}
		...
		if (is_array(@$_SESSION["sel_viewsaluddiagnostico_especialidad"])) {
			$this->LoadSelectionFromSession('especialidad');
		} elseif (@$_SESSION["sel_viewsaluddiagnostico_especialidad"] == EWR_INIT_VALUE) { // Select all
			$this->especialidad->SelectionList = "";
		}

		// Get resultado selected values
		if (is_array(@$_SESSION["sel_viewsaluddiagnostico_resultado"])) {
			$this->LoadSelectionFromSession('resultado');
		} elseif (@$_SESSION["sel_viewsaluddiagnostico_resultado"] == EWR_INIT_VALUE) { // Select all
			$this->resultado->SelectionList = "";
		}

		// Get diagnostico selected values
		if (is_array(@$_SESSION["sel_viewsaluddiagnostico_diagnostico"])) {
			$this->LoadSelectionFromSession('diagnostico');
		} elseif (@$_SESSION["sel_viewsaluddiagnostico_diagnostico"] == EWR_INIT_VALUE) { // Select all
			$this->diagnostico->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->diagnostico->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// diagnostico
			$CurrentValue = $this->diagnostico->CurrentValue;
			$ViewValue = &$this->diagnostico->ViewValue;
			$ViewAttrs = &$this->diagnostico->ViewAttrs;
			$CellAttrs = &$this->diagnostico->CellAttrs;
			$HrefValue = &$this->diagnostico->HrefValue;
			$LinkAttrs = &$this->diagnostico->LinkAttrs;
			$this->Cell_Rendered($this->diagnostico, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->diagnostico->Visible) $this->DtlColumnCount += 1;
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'viewsaluddiagnostico_especialidad')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'especialidad');

			// Clear extended filter for field resultado
			if ($this->ClearExtFilter == 'viewsaluddiagnostico_resultado')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'resultado');

			// Clear extended filter for field diagnostico
			if ($this->ClearExtFilter == 'viewsaluddiagnostico_diagnostico')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'diagnostico');
		...
		// Reset search command
			...
			$this->SetSessionFilterValues($this->diagnostico->SearchValue, $this->diagnostico->SearchOperator, $this->diagnostico->SearchCondition, $this->diagnostico->SearchValue2, $this->diagnostico->SearchOperator2, 'diagnostico'); // Field diagnostico
			...
			// Field diagnostico
			if ($this->GetFilterValues($this->diagnostico)) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionFilterValues($this->diagnostico); // Field diagnostico
		}
		...
		$this->BuildExtendedFilter($this->diagnostico, $sFilter, FALSE, TRUE); // Field diagnostico
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_viewsaluddiagnostico_especialidad'] = ($this->especialidad->SelectionList == "") ? EWR_INIT_VALUE : $this->especialidad->SelectionList;

			// Field resultado
			$sWrk = "";
			$this->BuildExtendedFilter($this->resultado, $sWrk);
			ewr_LoadSelectionFromFilter($this->resultado, $sWrk, $this->resultado->SelectionList);
			$_SESSION['sel_viewsaluddiagnostico_resultado'] = ($this->resultado->SelectionList == "") ? EWR_INIT_VALUE : $this->resultado->SelectionList;

			// Field diagnostico
			$sWrk = "";
			$this->BuildExtendedFilter($this->diagnostico, $sWrk);
			ewr_LoadSelectionFromFilter($this->diagnostico, $sWrk, $this->diagnostico->SelectionList);
			$_SESSION['sel_viewsaluddiagnostico_diagnostico'] = ($this->diagnostico->SelectionList == "") ? EWR_INIT_VALUE : $this->diagnostico->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_viewsaluddiagnostico_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_viewsaluddiagnostico_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_viewsaluddiagnostico_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_viewsaluddiagnostico_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_viewsaluddiagnostico_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_viewsaluddiagnostico_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_viewsaluddiagnostico_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_viewsaluddiagnostico_' . $parm] = $sv;
		$_SESSION['so_viewsaluddiagnostico_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_viewsaluddiagnostico_' . $parm] = $sv1;
		$_SESSION['so_viewsaluddiagnostico_' . $parm] = $so1;
		$_SESSION['sc_viewsaluddiagnostico_' . $parm] = $sc;
		$_SESSION['sv2_viewsaluddiagnostico_' . $parm] = $sv2;
		$_SESSION['so2_viewsaluddiagnostico_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_viewsaluddiagnostico_$parm"] = "";
		$_SESSION["rf_viewsaluddiagnostico_$parm"] = "";
		$_SESSION["rt_viewsaluddiagnostico_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_viewsaluddiagnostico_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_viewsaluddiagnostico_$parm"];
		$fld->RangeTo = @$_SESSION["rt_viewsaluddiagnostico_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field diagnostico
		$this->SetDefaultExtFilter($this->diagnostico, "LIKE", NULL, 'AND', "=", NULL);
		if (!$this->SearchCommand) $this->ApplyDefaultExtFilter($this->diagnostico);
		$sWrk = "";
		$this->BuildExtendedFilter($this->diagnostico, $sWrk, TRUE);
		ewr_LoadSelectionFromFilter($this->diagnostico, $sWrk, $this->diagnostico->DefaultSelectionList);
		if (!$this->SearchCommand) $this->diagnostico->SelectionList = $this->diagnostico->DefaultSelectionList;
		/**
		...
		*/
		...
		// Field diagnostico
		// $this->diagnostico->DefaultSelectionList = array("val1", "val2");
	...
	function CheckFilter() {
		...
		// Check diagnostico text filter
		if ($this->TextFilterApplied($this->diagnostico))
			return TRUE;

		// Check diagnostico popup filter
		if (!ewr_MatchedArray($this->diagnostico->DefaultSelectionList, $this->diagnostico->SelectionList))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($this->diagnostico, $sExtWrk);
		if (is_array($this->diagnostico->SelectionList))
			$sWrk = ewr_JoinArray($this->diagnostico->SelectionList, ", ", EWR_DATATYPE_STRING, 0, $this->DBID);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sWrk</span>";
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->diagnostico->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field diagnostico
		$sWrk = "";
		if ($this->diagnostico->SearchValue <> "" || $this->diagnostico->SearchValue2 <> "") {
			$sWrk = "\"sv_diagnostico\":\"" . ewr_JsEncode2($this->diagnostico->SearchValue) . "\"," .
				"\"so_diagnostico\":\"" . ewr_JsEncode2($this->diagnostico->SearchOperator) . "\"," .
				"\"sc_diagnostico\":\"" . ewr_JsEncode2($this->diagnostico->SearchCondition) . "\"," .
				"\"sv2_diagnostico\":\"" . ewr_JsEncode2($this->diagnostico->SearchValue2) . "\"," .
				"\"so2_diagnostico\":\"" . ewr_JsEncode2($this->diagnostico->SearchOperator2) . "\"";
		}
		if ($sWrk == "") {
			$sWrk = ($this->diagnostico->SelectionList <> EWR_INIT_VALUE) ? $this->diagnostico->SelectionList : "";
			if (is_array($sWrk))
				$sWrk = implode("||", $sWrk);
			if ($sWrk <> "")
				$sWrk = "\"sel_diagnostico\":\"" . ewr_JsEncode2($sWrk) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_especialidad", $filter)) {
			...
			$_SESSION["sel_viewsaluddiagnostico_especialidad"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "especialidad");
			$this->especialidad->SelectionList = "";
			$_SESSION["sel_viewsaluddiagnostico_especialidad"] = "";
		}
		...
		if (array_key_exists("sel_resultado", $filter)) {
			...
			$_SESSION["sel_viewsaluddiagnostico_resultado"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "resultado");
			$this->resultado->SelectionList = "";
			$_SESSION["sel_viewsaluddiagnostico_resultado"] = "";
		}

		// Field diagnostico
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_diagnostico", $filter) || array_key_exists("so_diagnostico", $filter) ||
			array_key_exists("sc_diagnostico", $filter) ||
			array_key_exists("sv2_diagnostico", $filter) || array_key_exists("so2_diagnostico", $filter)) {
			$this->SetSessionFilterValues(@$filter["sv_diagnostico"], @$filter["so_diagnostico"], @$filter["sc_diagnostico"], @$filter["sv2_diagnostico"], @$filter["so2_diagnostico"], "diagnostico");
			$bRestoreFilter = TRUE;
		}
		if (array_key_exists("sel_diagnostico", $filter)) {
			$sWrk = $filter["sel_diagnostico"];
			$sWrk = explode("||", $sWrk);
			$this->diagnostico->SelectionList = $sWrk;
			$_SESSION["sel_viewsaluddiagnostico_diagnostico"] = $sWrk;
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "diagnostico"); // Clear extended filter
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "diagnostico");
			$this->diagnostico->SelectionList = "";
			$_SESSION["sel_viewsaluddiagnostico_diagnostico"] = "";
		}
		return TRUE;
	}
	...
	function GetPopupFilter() {
		...
		if (!$this->ExtendedFilterExist($this->diagnostico)) {
			if (is_array($this->diagnostico->SelectionList)) {
				$sFilter = ewr_FilterSql($this->diagnostico, "`diagnostico`", EWR_DATATYPE_STRING, $this->DBID);

				// Call Page Filtering event
				$this->Page_Filtering($this->diagnostico, $sFilter, "popup");
				$this->diagnostico->CurrentFilter = $sFilter;
				ewr_AddFilter($sWrk, $sFilter);
			}
		}
		return $sWrk;
...
// Create page object
if (!isset($viewsaluddiagnostico_rpt)) $viewsaluddiagnostico_rpt = new crviewsaluddiagnostico_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$viewsaluddiagnostico_rpt;
...
// Create page object
var viewsaluddiagnostico_rpt = new ewr_Page("viewsaluddiagnostico_rpt");

// Page properties
viewsaluddiagnostico_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = viewsaluddiagnostico_rpt.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fviewsaluddiagnosticorpt = new ewr_Form("fviewsaluddiagnosticorpt");

// Validate method
fviewsaluddiagnosticorpt.Validate = function() {
...
fviewsaluddiagnosticorpt.Form_CustomValidate = 
...
fviewsaluddiagnosticorpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fviewsaluddiagnosticorpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="fviewsaluddiagnosticorpt" id="fviewsaluddiagnosticorpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fviewsaluddiagnosticorpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="viewsaluddiagnostico" data-field="x_especialidad" id="sv_especialidad" name="sv_especialidad" size="30" maxlength="20" placeholder="<?php echo $Page->especialidad->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->especialidad->SearchValue) ?>"<?php echo $Page->especialidad->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewsaluddiagnostico" data-field="x_nombreotros" id="sv_nombreotros" name="sv_nombreotros" size="30" maxlength="100" placeholder="<?php echo $Page->nombreotros->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->nombreotros->SearchValue) ?>"<?php echo $Page->nombreotros->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewsaluddiagnostico" data-field="x_nombreneonatal" id="sv_nombreneonatal" name="sv_nombreneonatal" size="30" maxlength="100" placeholder="<?php echo $Page->nombreneonatal->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->nombreneonatal->SearchValue) ?>"<?php echo $Page->nombreneonatal->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewsaluddiagnostico" data-field="x_resultado" id="sv_resultado" name="sv_resultado" size="30" maxlength="100" placeholder="<?php echo $Page->resultado->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->resultado->SearchValue) ?>"<?php echo $Page->resultado->EditAttributes() ?>>
</span>
</div>
</div>
<div id="r_5" class="ewRow">
<div id="c_diagnostico" class="ewCell form-group">
	<label for="sv_diagnostico" class="ewSearchCaption ewLabel"><?php echo $Page->diagnostico->FldCaption() ?></label>
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_diagnostico" id="so_diagnostico" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->diagnostico->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="viewsaluddiagnostico" data-field="x_diagnostico" id="sv_diagnostico" name="sv_diagnostico" size="30" maxlength="100" placeholder="<?php echo $Page->diagnostico->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->diagnostico->SearchValue) ?>"<?php echo $Page->diagnostico->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fviewsaluddiagnosticorpt.Init();
fviewsaluddiagnosticorpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_viewsaluddiagnostico" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="especialidad"><div class="viewsaluddiagnostico_especialidad"><span class="ewTableHeaderCaption"><?php echo $Page->especialidad->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="especialidad">
<?php if ($Page->SortUrl($Page->especialidad) == "") { ?>
		<div class="ewTableHeaderBtn viewsaluddiagnostico_especialidad">
			<span class="ewTableHeaderCaption"><?php echo $Page->especialidad->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaluddiagnostico_especialidad', range: false, from: '<?php echo $Page->especialidad->RangeFrom; ?>', to: '<?php echo $Page->especialidad->RangeTo; ?>', url: 'viewsaluddiagnosticorpt.php' });" id="x_especialidad<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaluddiagnostico_especialidad" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->especialidad) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->especialidad->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombreotros"><div class="viewsaluddiagnostico_nombreotros"><span class="ewTableHeaderCaption"><?php echo $Page->nombreotros->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombreotros">
<?php if ($Page->SortUrl($Page->nombreotros) == "") { ?>
		<div class="ewTableHeaderBtn viewsaluddiagnostico_nombreotros">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreotros->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaluddiagnostico_nombreotros" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombreotros) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreotros->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombreneonatal"><div class="viewsaluddiagnostico_nombreneonatal"><span class="ewTableHeaderCaption"><?php echo $Page->nombreneonatal->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombreneonatal">
<?php if ($Page->SortUrl($Page->nombreneonatal) == "") { ?>
		<div class="ewTableHeaderBtn viewsaluddiagnostico_nombreneonatal">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreneonatal->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaluddiagnostico_nombreneonatal" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombreneonatal) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreneonatal->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="resultado"><div class="viewsaluddiagnostico_resultado"><span class="ewTableHeaderCaption"><?php echo $Page->resultado->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="resultado">
<?php if ($Page->SortUrl($Page->resultado) == "") { ?>
		<div class="ewTableHeaderBtn viewsaluddiagnostico_resultado">
			<span class="ewTableHeaderCaption"><?php echo $Page->resultado->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaluddiagnostico_resultado', range: false, from: '<?php echo $Page->resultado->RangeFrom; ?>', to: '<?php echo $Page->resultado->RangeTo; ?>', url: 'viewsaluddiagnosticorpt.php' });" id="x_resultado<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaluddiagnostico_resultado" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->resultado) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->resultado->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->diagnostico->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="diagnostico"><div class="viewsaluddiagnostico_diagnostico"><span class="ewTableHeaderCaption"><?php echo $Page->diagnostico->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="diagnostico">
<?php if ($Page->SortUrl($Page->diagnostico) == "") { ?>
		<div class="ewTableHeaderBtn viewsaluddiagnostico_diagnostico">
			<span class="ewTableHeaderCaption"><?php echo $Page->diagnostico->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaluddiagnostico_diagnostico', range: false, from: '<?php echo $Page->diagnostico->RangeFrom; ?>', to: '<?php echo $Page->diagnostico->RangeTo; ?>', url: 'viewsaluddiagnosticorpt.php' });" id="x_diagnostico<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaluddiagnostico_diagnostico" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->diagnostico) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->diagnostico->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->diagnostico->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->diagnostico->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaluddiagnostico_diagnostico', range: false, from: '<?php echo $Page->diagnostico->RangeFrom; ?>', to: '<?php echo $Page->diagnostico->RangeTo; ?>', url: 'viewsaluddiagnosticorpt.php' });" id="x_diagnostico<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
...
<?php } ?>
<?php if ($Page->diagnostico->Visible) { ?>
		<td data-field="diagnostico"<?php echo $Page->diagnostico->CellAttributes() ?>>
<span<?php echo $Page->diagnostico->ViewAttributes() ?>><?php echo $Page->diagnostico->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "viewsaluddiagnosticorptpager.php" ?>
