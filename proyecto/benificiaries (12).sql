...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 03, 2019 at 01:01 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewdocente`  AS  select `departamento`.`nombre` AS `deoartamento`,`unidadeducativa`.`nombre` AS `unidadeducativa`,`docente`.`nombres` AS `nombres`,`docente`.`apellidopaterno` AS `apellidopaterno`,`docente`.`apellidomaterno` AS `apellidomaterno`,`docente`.`nrodiscapacidad` AS `nrodiscapacidad`,`docente`.`ci` AS `ci`,`docente`.`fechanacimiento` AS `fechanacimiento`,`docente`.`sexo` AS `sexo`,`docente`.`celular` AS `celular`,`docente`.`materias` AS `materias`,`docente`.`discapacidad` AS `discapacidad`,`docente`.`tipodiscapacidad` AS `tipodiscapacidad`,`centros`.`nombreinstitucion` AS `nombreinstitucion` from (((`docente` join `departamento` on((`departamento`.`id` = `docente`.`id_departamento`))) join `unidadeducativa` on((`unidadeducativa`.`id` = `docente`.`unidadeducativa`))) join `centros` on((`centros`.`id` = `docente`.`id_centro`))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewparticipante`  AS  select `sector`.`nombre` AS `sector`,`actividad`.`nombreactividad` AS `actividad`,`categoria`.`nombre` AS `categoria`,`centros`.`nombreinstitucion` AS `nombreinstitucion`,`participante`.`nombre` AS `nombre`,`participante`.`apellidopaterno` AS `apellidopaterno`,`participante`.`apellidomaterno` AS `apellidomaterno`,`participante`.`fecha_nacimiento` AS `fecha_nacimiento`,`participante`.`sexo` AS `sexo`,`participante`.`ci` AS `ci`,`participante`.`nrodiscapacidad` AS `nrodiscapacidad`,`participante`.`celular` AS `celular`,`participante`.`direcciondomicilio` AS `direcciondomicilio`,`participante`.`ocupacion` AS `ocupacion`,`participante`.`email` AS `email`,`participante`.`cargo` AS `cargo`,`participante`.`nivelestudio` AS `nivelestudio`,`participante`.`observaciones` AS `observaciones` from ((((`participante` join `sector` on((`sector`.`id` = `participante`.`id_sector`))) join `actividad` on((`actividad`.`id` = `participante`.`id_actividad`))) join `categoria` on((`categoria`.`id` = `participante`.`id_categoria`))) join `centros` on((`centros`.`id` = `participante`.`id_centro`))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewunidadeducativa`  AS  select `centros`.`nombreinstitucion` AS `centro`,`unidadeducativa`.`nombre` AS `unidadeducativa`,`unidadeducativa`.`codigo_sie` AS `codigo_sie`,`departamento`.`nombre` AS `departamento`,`municipio`.`nombre` AS `municipio`,`provincia`.`nombre` AS `principio`,`unidadeducativa`.`direccion` AS `direccion`,`unidadeducativa`.`telefono` AS `telefono`,`unidadeducativa`.`email` AS `email` from ((((`unidadeducativa` join `centros` on((`centros`.`id` = `unidadeducativa`.`id_centro`))) join `departamento` on((`departamento`.`id` = `unidadeducativa`.`departamento`))) join `provincia` on((`provincia`.`id` = `unidadeducativa`.`provincia`))) join `municipio` on((`municipio`.`id` = `unidadeducativa`.`municipio`))) ;
