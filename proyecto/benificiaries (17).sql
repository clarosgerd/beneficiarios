...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 11, 2019 at 10:21 PM
...
--
-- Stand-in structure for view `viewmarcologicosummary`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `viewsalsudapoderado`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `viewsaludescolar`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `viewsaludneonatal`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `viewsaludotros`
-- (See below for the actual view)
--
...
--
-- Stand-in structure for view `viewsaludreferencia`
-- (See below for the actual view)
--
...
--
-- Structure for view `viewmarcologicosummary`
--
...
--
-- Structure for view `viewsalsudapoderado`
--
DROP TABLE IF EXISTS `viewsalsudapoderado`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewsalsudapoderado`  AS  select `apoderado`.`apellidopaterno` AS `apellidopaterno`,`apoderado`.`apellidomaterno` AS `apellidomaterno`,`apoderado`.`nombres` AS `nombres`,`apoderado`.`ci` AS `ci`,`apoderado`.`fechanacimiento` AS `fechanacimiento`,`apoderado`.`sexo` AS `sexo`,`apoderado`.`dirección` AS `dirección`,`apoderado`.`celular` AS `celular`,`apoderado`.`ocupación` AS `ocupación`,`apoderado`.`observaciones` AS `observaciones`,`apoderado`.`parentesco` AS `parentesco` from `apoderado` ;

-- --------------------------------------------------------

--
-- Structure for view `viewsaludescolar`
--
DROP TABLE IF EXISTS `viewsaludescolar`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewsaludescolar`  AS  select `escolar`.`codigorude` AS `codigorude`,`escolar`.`codigorude_es` AS `codigorude_es`,`escolar`.`fecha` AS `fecha`,`unidadeducativa`.`nombre` AS `unidad eductiva`,`escolar`.`apellidopaterno` AS `apellidopaterno`,`escolar`.`apellidomaterno` AS `apellidomaterno`,`escolar`.`nombres` AS `nombres`,`escolar`.`ci` AS `ci`,`escolar`.`nrodiscapacidad` AS `nrodiscapacidad`,`escolar`.`fechanacimiento` AS `fechanacimiento`,`escolar`.`sexo` AS `sexo`,`curso`.`curso` AS `curso`,`discapacidad`.`nombre` AS `discapcidad`,`tipodiscapacidad`.`nombre` AS `tipo`,`escolar`.`resultado` AS `resultado`,`escolar`.`resultadotamizaje` AS `resultadotamizaje`,`tapon`.`nombre` AS `nombre`,`escolar`.`tapodonde` AS `tapodonde`,`escolar`.`repetirprueba` AS `repetirprueba`,`escolar`.`observaciones` AS `observaciones`,`apoderado`.`parentesco` AS `parentesco`,`referencia`.`nombrescompleto` AS `nombrescompleto` from (((((((`escolar` join `unidadeducativa` on((`unidadeducativa`.`id` = `escolar`.`unidadeducativa`))) join `curso` on((`curso`.`id` = `escolar`.`curso`))) join `discapacidad` on((`discapacidad`.`id` = `escolar`.`id_discapacidad`))) join `tipodiscapacidad` on((`tipodiscapacidad`.`id` = `escolar`.`id_tipodiscapacidad`))) join `tapon` on((`tapon`.`id` = `escolar`.`tapon`))) join `apoderado` on((`apoderado`.`id` = `escolar`.`id_apoderado`))) join `referencia` on((`referencia`.`id` = `escolar`.`id_referencia`))) ;

-- --------------------------------------------------------

--
-- Structure for view `viewsaludneonatal`
--
DROP TABLE IF EXISTS `viewsaludneonatal`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewsaludneonatal`  AS  select `neonatal`.`fecha_tamizaje` AS `fecha_tamizaje`,`institucionesdesalud`.`nombre` AS `nombre`,`neonatal`.`nombre` AS `nombre1`,`neonatal`.`apellidomaterno` AS `apellidomaterno`,`neonatal`.`apellidopaterno` AS `apellidopaterno`,`neonatal`.`ci` AS `ci`,`neonatal`.`fecha_nacimiento` AS `fecha_nacimiento`,`neonatal`.`dias` AS `dias`,`neonatal`.`semanas` AS `semanas`,`neonatal`.`meses` AS `meses`,`neonatal`.`sexo` AS `sexo`,`discapacidad`.`nombre` AS `nombre2`,`tipodiscapacidad`.`nombre` AS `nombre3`,`neonatal`.`resultado` AS `resultado`,`neonatal`.`resultadotamizaje` AS `resultadotamizaje`,`neonatal`.`tapon` AS `tapon`,`neonatal`.`tipo` AS `tipo`,`neonatal`.`repetirprueba` AS `repetirprueba`,`neonatal`.`observaciones` AS `observaciones`,`apoderado`.`parentesco` AS `parentesco`,`referencia`.`nombrescompleto` AS `nombrescompleto` from (((((`neonatal` join `institucionesdesalud` on((`institucionesdesalud`.`id` = `neonatal`.`id_centro`))) join `discapacidad` on((`discapacidad`.`id` = `neonatal`.`discapacidad`))) join `tipodiscapacidad` on((`tipodiscapacidad`.`id` = `neonatal`.`id_tipodiscapacidad`))) join `apoderado` on((`apoderado`.`id` = `neonatal`.`id_apoderado`))) join `referencia` on((`referencia`.`id` = `neonatal`.`id_referencia`))) ;

-- --------------------------------------------------------

--
-- Structure for view `viewsaludotros`
--
DROP TABLE IF EXISTS `viewsaludotros`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewsaludotros`  AS  select `actividad`.`nombreactividad` AS `nombreactividad`,`otros`.`apellidopaterno` AS `apellidopaterno`,`otros`.`apellidomaterno` AS `apellidomaterno`,`otros`.`nombre` AS `nombre`,`otros`.`nrodiscapacidad` AS `nrodiscapacidad`,`otros`.`ci` AS `ci`,`otros`.`fecha_nacimiento` AS `fecha_nacimiento`,`otros`.`sexo` AS `sexo`,`otros`.`nivelestudio` AS `nivelestudio`,`discapacidad`.`nombre` AS `discacidad`,`tipodiscapacidad`.`nombre` AS `tipo discapacidad`,`otros`.`resultado` AS `resultado`,`otros`.`resultadotamizaje` AS `resultadotamizaje`,`otros`.`tapon` AS `tapon`,`otros`.`tipo` AS `tipo tapo`,`otros`.`repetirprueba` AS `repetirprueba`,`otros`.`observaciones` AS `observaciones`,`apoderado`.`parentesco` AS `parentesco`,`referencia`.`nombrescompleto` AS `nombrescompleto` from (((((`otros` join `actividad` on((`actividad`.`id` = `otros`.`id_actividad`))) join `discapacidad` on((`discapacidad`.`id` = `otros`.`id_discapacidad`))) join `tipodiscapacidad` on((`tipodiscapacidad`.`id` = `otros`.`id_tipodiscapacidad`))) join `apoderado` on((`apoderado`.`id` = `otros`.`id_apoderado`))) join `referencia` on((`referencia`.`id` = `otros`.`id_referencia`))) ;

-- --------------------------------------------------------

--
-- Structure for view `viewsaludreferencia`
--
DROP TABLE IF EXISTS `viewsaludreferencia`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewsaludreferencia`  AS  select `medio`.`nombre` AS `medio`,`referencia`.`nombrescompleto` AS `nombrescompleto`,`referencia`.`nombrescentromedico` AS `nombrescentromedico`,`referencia`.`dirección` AS `dirección`,`referencia`.`telefono` AS `telefono` from (`referencia` join `medio` on((`medio`.`id` = `referencia`.`id_medio`))) ;
