...
<?php include_once "personarptinfo.php" ?>
<?php
...
$persona_rpt = NULL; // Initialize page object first

class crpersona_rpt extends crpersona {
	...
	function __construct() {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'persona', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fpersonarpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'persona');
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_persona\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_persona',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fpersonarpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fpersonarpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fpersonarpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['tipopersona'] = ewr_Conv($rs->fields('tipopersona'), 200);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			...
			$this->tipopersona->setDbValue($rs->fields('tipopersona'));
			...
			$this->Val[8] = $this->tipopersona->CurrentValue;
		} else {
			$this->id->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->tipopersona->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// tipopersona
			$CurrentValue = $this->tipopersona->CurrentValue;
			$ViewValue = &$this->tipopersona->ViewValue;
			$ViewAttrs = &$this->tipopersona->ViewAttrs;
			$CellAttrs = &$this->tipopersona->CellAttrs;
			$HrefValue = &$this->tipopersona->HrefValue;
			$LinkAttrs = &$this->tipopersona->LinkAttrs;
			$this->Cell_Rendered($this->tipopersona, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->tipopersona->Visible) $this->DtlColumnCount += 1;
	}
...
// Create page object
if (!isset($persona_rpt)) $persona_rpt = new crpersona_rpt();
...
// Create page object
var persona_rpt = new ewr_Page("persona_rpt");

// Page properties
persona_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = persona_rpt.PageID;
</script>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_persona" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id"><div class="persona_id"><span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id">
<?php if ($Page->SortUrl($Page->id) == "") { ?>
		<div class="ewTableHeaderBtn persona_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombreinstitucion"><div class="persona_nombreinstitucion"><span class="ewTableHeaderCaption"><?php echo $Page->nombreinstitucion->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombreinstitucion">
<?php if ($Page->SortUrl($Page->nombreinstitucion) == "") { ?>
		<div class="ewTableHeaderBtn persona_nombreinstitucion">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreinstitucion->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona_nombreinstitucion" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombreinstitucion) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreinstitucion->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="apellidopaterno"><div class="persona_apellidopaterno"><span class="ewTableHeaderCaption"><?php echo $Page->apellidopaterno->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="apellidopaterno">
<?php if ($Page->SortUrl($Page->apellidopaterno) == "") { ?>
		<div class="ewTableHeaderBtn persona_apellidopaterno">
			<span class="ewTableHeaderCaption"><?php echo $Page->apellidopaterno->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona_apellidopaterno" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->apellidopaterno) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->apellidopaterno->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="apellidomaterno"><div class="persona_apellidomaterno"><span class="ewTableHeaderCaption"><?php echo $Page->apellidomaterno->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="apellidomaterno">
<?php if ($Page->SortUrl($Page->apellidomaterno) == "") { ?>
		<div class="ewTableHeaderBtn persona_apellidomaterno">
			<span class="ewTableHeaderCaption"><?php echo $Page->apellidomaterno->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona_apellidomaterno" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->apellidomaterno) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->apellidomaterno->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombre"><div class="persona_nombre"><span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombre">
<?php if ($Page->SortUrl($Page->nombre) == "") { ?>
		<div class="ewTableHeaderBtn persona_nombre">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona_nombre" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombre) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="telefono"><div class="persona_telefono"><span class="ewTableHeaderCaption"><?php echo $Page->telefono->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="telefono">
<?php if ($Page->SortUrl($Page->telefono) == "") { ?>
		<div class="ewTableHeaderBtn persona_telefono">
			<span class="ewTableHeaderCaption"><?php echo $Page->telefono->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona_telefono" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->telefono) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->telefono->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="_email"><div class="persona__email"><span class="ewTableHeaderCaption"><?php echo $Page->_email->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="_email">
<?php if ($Page->SortUrl($Page->_email) == "") { ?>
		<div class="ewTableHeaderBtn persona__email">
			<span class="ewTableHeaderCaption"><?php echo $Page->_email->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona__email" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->_email) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->_email->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->tipopersona->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tipopersona"><div class="persona_tipopersona"><span class="ewTableHeaderCaption"><?php echo $Page->tipopersona->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tipopersona">
<?php if ($Page->SortUrl($Page->tipopersona) == "") { ?>
		<div class="ewTableHeaderBtn persona_tipopersona">
			<span class="ewTableHeaderCaption"><?php echo $Page->tipopersona->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer persona_tipopersona" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tipopersona) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tipopersona->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->tipopersona->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->tipopersona->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php } ?>
<?php if ($Page->tipopersona->Visible) { ?>
		<td data-field="tipopersona"<?php echo $Page->tipopersona->CellAttributes() ?>>
<span<?php echo $Page->tipopersona->ViewAttributes() ?>><?php echo $Page->tipopersona->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "personarptpager.php" ?>
