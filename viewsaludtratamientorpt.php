...
<?php include_once "viewsaludtratamientorptinfo.php" ?>
<?php
...
$viewsaludtratamiento_rpt = NULL; // Initialize page object first

class crviewsaludtratamiento_rpt extends crviewsaludtratamiento {
	...
	// Page object name
	var $PageObjName = 'viewsaludtratamiento_rpt';
	...
	function __construct() {
		...
		// Table object (viewsaludtratamiento)
		if (!isset($GLOBALS["viewsaludtratamiento"])) {
			$GLOBALS["viewsaludtratamiento"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["viewsaludtratamiento"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'viewsaludtratamiento', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fviewsaludtratamientorpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'viewsaludtratamiento');
		...
		$this->tratamiento->PlaceHolder = $this->tratamiento->FldCaption();
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fviewsaludtratamientorpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fviewsaludtratamientorpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fviewsaludtratamientorpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->tratamiento->SelectionList = "";
		$this->tratamiento->DefaultSelectionList = "";
		$this->tratamiento->ValueList = "";
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['tratamiento'] = ewr_Conv($rs->fields('tratamiento'), 200);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'viewsaludtratamiento_especialidad') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			// Build distinct values for tratamiento
			if ($popupname == 'viewsaludtratamiento_tratamiento') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;

				// Call Page Filtering event
				$this->Page_Filtering($this->tratamiento, $sFilter, "popup");
				$sSql = ewr_BuildReportSql($this->tratamiento->SqlSelect, $this->getSqlWhere(), $this->getSqlGroupBy(), $this->getSqlHaving(), $this->tratamiento->SqlOrderBy, $sFilter, "");
				$rswrk = $conn->Execute($sSql);
				while ($rswrk && !$rswrk->EOF) {
					$this->tratamiento->setDbValue($rswrk->fields[0]);
					$this->tratamiento->ViewValue = @$rswrk->fields[1];
					if (is_null($this->tratamiento->CurrentValue)) {
						$bNullValue = TRUE;
					} elseif ($this->tratamiento->CurrentValue == "") {
						$bEmptyValue = TRUE;
					} else {
						ewr_SetupDistinctValues($this->tratamiento->ValueList, $this->tratamiento->CurrentValue, $this->tratamiento->ViewValue, FALSE, $this->tratamiento->FldDelimiter);
					}
					$rswrk->MoveNext();
				}
				if ($rswrk)
					$rswrk->Close();
				if ($bEmptyValue)
					ewr_SetupDistinctValues($this->tratamiento->ValueList, EWR_EMPTY_VALUE, $ReportLanguage->Phrase("EmptyLabel"), FALSE);
				if ($bNullValue)
					ewr_SetupDistinctValues($this->tratamiento->ValueList, EWR_NULL_VALUE, $ReportLanguage->Phrase("NullLabel"), FALSE);
				$fld = &$this->tratamiento;
			}
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				...
				$this->ClearSessionSelection('tratamiento');
				$this->ResetPager();
			}
		}
		...
		if (is_array(@$_SESSION["sel_viewsaludtratamiento_especialidad"])) {
			$this->LoadSelectionFromSession('especialidad');
		} elseif (@$_SESSION["sel_viewsaludtratamiento_especialidad"] == EWR_INIT_VALUE) { // Select all
			$this->especialidad->SelectionList = "";
		}

		// Get tratamiento selected values
		if (is_array(@$_SESSION["sel_viewsaludtratamiento_tratamiento"])) {
			$this->LoadSelectionFromSession('tratamiento');
		} elseif (@$_SESSION["sel_viewsaludtratamiento_tratamiento"] == EWR_INIT_VALUE) { // Select all
			$this->tratamiento->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->tratamiento->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// tratamiento
			$CurrentValue = $this->tratamiento->CurrentValue;
			$ViewValue = &$this->tratamiento->ViewValue;
			$ViewAttrs = &$this->tratamiento->ViewAttrs;
			$CellAttrs = &$this->tratamiento->CellAttrs;
			$HrefValue = &$this->tratamiento->HrefValue;
			$LinkAttrs = &$this->tratamiento->LinkAttrs;
			$this->Cell_Rendered($this->tratamiento, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->tratamiento->Visible) $this->DtlColumnCount += 1;
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'viewsaludtratamiento_especialidad')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'especialidad');

			// Clear extended filter for field tratamiento
			if ($this->ClearExtFilter == 'viewsaludtratamiento_tratamiento')
				$this->SetSessionFilterValues('', '=', 'AND', '', '=', 'tratamiento');
		...
		// Reset search command
			...
			$this->SetSessionFilterValues($this->tratamiento->SearchValue, $this->tratamiento->SearchOperator, $this->tratamiento->SearchCondition, $this->tratamiento->SearchValue2, $this->tratamiento->SearchOperator2, 'tratamiento'); // Field tratamiento
			...
			// Field tratamiento
			if ($this->GetFilterValues($this->tratamiento)) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionFilterValues($this->tratamiento); // Field tratamiento
		}
		...
		$this->BuildExtendedFilter($this->tratamiento, $sFilter, FALSE, TRUE); // Field tratamiento
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_viewsaludtratamiento_especialidad'] = ($this->especialidad->SelectionList == "") ? EWR_INIT_VALUE : $this->especialidad->SelectionList;

			// Field tratamiento
			$sWrk = "";
			$this->BuildExtendedFilter($this->tratamiento, $sWrk);
			ewr_LoadSelectionFromFilter($this->tratamiento, $sWrk, $this->tratamiento->SelectionList);
			$_SESSION['sel_viewsaludtratamiento_tratamiento'] = ($this->tratamiento->SelectionList == "") ? EWR_INIT_VALUE : $this->tratamiento->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_viewsaludtratamiento_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_viewsaludtratamiento_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_viewsaludtratamiento_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_viewsaludtratamiento_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_viewsaludtratamiento_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_viewsaludtratamiento_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_viewsaludtratamiento_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_viewsaludtratamiento_' . $parm] = $sv;
		$_SESSION['so_viewsaludtratamiento_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_viewsaludtratamiento_' . $parm] = $sv1;
		$_SESSION['so_viewsaludtratamiento_' . $parm] = $so1;
		$_SESSION['sc_viewsaludtratamiento_' . $parm] = $sc;
		$_SESSION['sv2_viewsaludtratamiento_' . $parm] = $sv2;
		$_SESSION['so2_viewsaludtratamiento_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_viewsaludtratamiento_$parm"] = "";
		$_SESSION["rf_viewsaludtratamiento_$parm"] = "";
		$_SESSION["rt_viewsaludtratamiento_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_viewsaludtratamiento_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_viewsaludtratamiento_$parm"];
		$fld->RangeTo = @$_SESSION["rt_viewsaludtratamiento_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field tratamiento
		$this->SetDefaultExtFilter($this->tratamiento, "LIKE", NULL, 'AND', "=", NULL);
		if (!$this->SearchCommand) $this->ApplyDefaultExtFilter($this->tratamiento);
		$sWrk = "";
		$this->BuildExtendedFilter($this->tratamiento, $sWrk, TRUE);
		ewr_LoadSelectionFromFilter($this->tratamiento, $sWrk, $this->tratamiento->DefaultSelectionList);
		if (!$this->SearchCommand) $this->tratamiento->SelectionList = $this->tratamiento->DefaultSelectionList;
		/**
		...
		*/
		...
		// Field tratamiento
		// $this->tratamiento->DefaultSelectionList = array("val1", "val2");
	...
	function CheckFilter() {
		...
		// Check tratamiento text filter
		if ($this->TextFilterApplied($this->tratamiento))
			return TRUE;

		// Check tratamiento popup filter
		if (!ewr_MatchedArray($this->tratamiento->DefaultSelectionList, $this->tratamiento->SelectionList))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($this->tratamiento, $sExtWrk);
		if (is_array($this->tratamiento->SelectionList))
			$sWrk = ewr_JoinArray($this->tratamiento->SelectionList, ", ", EWR_DATATYPE_STRING, 0, $this->DBID);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sWrk</span>";
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->tratamiento->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field tratamiento
		$sWrk = "";
		if ($this->tratamiento->SearchValue <> "" || $this->tratamiento->SearchValue2 <> "") {
			$sWrk = "\"sv_tratamiento\":\"" . ewr_JsEncode2($this->tratamiento->SearchValue) . "\"," .
				"\"so_tratamiento\":\"" . ewr_JsEncode2($this->tratamiento->SearchOperator) . "\"," .
				"\"sc_tratamiento\":\"" . ewr_JsEncode2($this->tratamiento->SearchCondition) . "\"," .
				"\"sv2_tratamiento\":\"" . ewr_JsEncode2($this->tratamiento->SearchValue2) . "\"," .
				"\"so2_tratamiento\":\"" . ewr_JsEncode2($this->tratamiento->SearchOperator2) . "\"";
		}
		if ($sWrk == "") {
			$sWrk = ($this->tratamiento->SelectionList <> EWR_INIT_VALUE) ? $this->tratamiento->SelectionList : "";
			if (is_array($sWrk))
				$sWrk = implode("||", $sWrk);
			if ($sWrk <> "")
				$sWrk = "\"sel_tratamiento\":\"" . ewr_JsEncode2($sWrk) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_especialidad", $filter)) {
			...
			$_SESSION["sel_viewsaludtratamiento_especialidad"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "especialidad");
			$this->especialidad->SelectionList = "";
			$_SESSION["sel_viewsaludtratamiento_especialidad"] = "";
		}
		...
		// Field tratamiento
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_tratamiento", $filter) || array_key_exists("so_tratamiento", $filter) ||
			array_key_exists("sc_tratamiento", $filter) ||
			array_key_exists("sv2_tratamiento", $filter) || array_key_exists("so2_tratamiento", $filter)) {
			$this->SetSessionFilterValues(@$filter["sv_tratamiento"], @$filter["so_tratamiento"], @$filter["sc_tratamiento"], @$filter["sv2_tratamiento"], @$filter["so2_tratamiento"], "tratamiento");
			$bRestoreFilter = TRUE;
		}
		if (array_key_exists("sel_tratamiento", $filter)) {
			$sWrk = $filter["sel_tratamiento"];
			$sWrk = explode("||", $sWrk);
			$this->tratamiento->SelectionList = $sWrk;
			$_SESSION["sel_viewsaludtratamiento_tratamiento"] = $sWrk;
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "tratamiento"); // Clear extended filter
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "tratamiento");
			$this->tratamiento->SelectionList = "";
			$_SESSION["sel_viewsaludtratamiento_tratamiento"] = "";
		}
		return TRUE;
	}
	...
	function GetPopupFilter() {
		...
		if (!$this->ExtendedFilterExist($this->tratamiento)) {
			if (is_array($this->tratamiento->SelectionList)) {
				$sFilter = ewr_FilterSql($this->tratamiento, "`tratamiento`", EWR_DATATYPE_STRING, $this->DBID);

				// Call Page Filtering event
				$this->Page_Filtering($this->tratamiento, $sFilter, "popup");
				$this->tratamiento->CurrentFilter = $sFilter;
				ewr_AddFilter($sWrk, $sFilter);
			}
		}
		return $sWrk;
	...
	function GetSort($options = array()) {
		...
		// Check for an Order parameter
			...
			$this->UpdateSort($this->tratamiento); // tratamiento
...
// Create page object
if (!isset($viewsaludtratamiento_rpt)) $viewsaludtratamiento_rpt = new crviewsaludtratamiento_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$viewsaludtratamiento_rpt;
...
// Create page object
var viewsaludtratamiento_rpt = new ewr_Page("viewsaludtratamiento_rpt");

// Page properties
viewsaludtratamiento_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = viewsaludtratamiento_rpt.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fviewsaludtratamientorpt = new ewr_Form("fviewsaludtratamientorpt");

// Validate method
fviewsaludtratamientorpt.Validate = function() {
...
fviewsaludtratamientorpt.Form_CustomValidate = 
...
fviewsaludtratamientorpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fviewsaludtratamientorpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="fviewsaludtratamientorpt" id="fviewsaludtratamientorpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fviewsaludtratamientorpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="viewsaludtratamiento" data-field="x_especialidad" id="sv_especialidad" name="sv_especialidad" size="30" maxlength="20" placeholder="<?php echo $Page->especialidad->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->especialidad->SearchValue) ?>"<?php echo $Page->especialidad->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewsaludtratamiento" data-field="x_nombreotros" id="sv_nombreotros" name="sv_nombreotros" size="30" maxlength="100" placeholder="<?php echo $Page->nombreotros->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->nombreotros->SearchValue) ?>"<?php echo $Page->nombreotros->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="viewsaludtratamiento" data-field="x_nombreneonatal" id="sv_nombreneonatal" name="sv_nombreneonatal" size="30" maxlength="100" placeholder="<?php echo $Page->nombreneonatal->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->nombreneonatal->SearchValue) ?>"<?php echo $Page->nombreneonatal->EditAttributes() ?>>
</span>
</div>
</div>
<div id="r_4" class="ewRow">
<div id="c_tratamiento" class="ewCell form-group">
	<label for="sv_tratamiento" class="ewSearchCaption ewLabel"><?php echo $Page->tratamiento->FldCaption() ?></label>
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_tratamiento" id="so_tratamiento" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->tratamiento->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="viewsaludtratamiento" data-field="x_tratamiento" id="sv_tratamiento" name="sv_tratamiento" size="30" maxlength="100" placeholder="<?php echo $Page->tratamiento->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tratamiento->SearchValue) ?>"<?php echo $Page->tratamiento->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fviewsaludtratamientorpt.Init();
fviewsaludtratamientorpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_viewsaludtratamiento" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="especialidad"><div class="viewsaludtratamiento_especialidad"><span class="ewTableHeaderCaption"><?php echo $Page->especialidad->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="especialidad">
<?php if ($Page->SortUrl($Page->especialidad) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludtratamiento_especialidad">
			<span class="ewTableHeaderCaption"><?php echo $Page->especialidad->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaludtratamiento_especialidad', range: false, from: '<?php echo $Page->especialidad->RangeFrom; ?>', to: '<?php echo $Page->especialidad->RangeTo; ?>', url: 'viewsaludtratamientorpt.php' });" id="x_especialidad<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludtratamiento_especialidad" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->especialidad) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->especialidad->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombreotros"><div class="viewsaludtratamiento_nombreotros"><span class="ewTableHeaderCaption"><?php echo $Page->nombreotros->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombreotros">
<?php if ($Page->SortUrl($Page->nombreotros) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludtratamiento_nombreotros">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreotros->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludtratamiento_nombreotros" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombreotros) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreotros->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombreneonatal"><div class="viewsaludtratamiento_nombreneonatal"><span class="ewTableHeaderCaption"><?php echo $Page->nombreneonatal->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombreneonatal">
<?php if ($Page->SortUrl($Page->nombreneonatal) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludtratamiento_nombreneonatal">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreneonatal->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludtratamiento_nombreneonatal" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombreneonatal) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreneonatal->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->tratamiento->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tratamiento"><div class="viewsaludtratamiento_tratamiento"><span class="ewTableHeaderCaption"><?php echo $Page->tratamiento->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tratamiento">
<?php if ($Page->SortUrl($Page->tratamiento) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludtratamiento_tratamiento">
			<span class="ewTableHeaderCaption"><?php echo $Page->tratamiento->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaludtratamiento_tratamiento', range: false, from: '<?php echo $Page->tratamiento->RangeFrom; ?>', to: '<?php echo $Page->tratamiento->RangeTo; ?>', url: 'viewsaludtratamientorpt.php' });" id="x_tratamiento<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludtratamiento_tratamiento" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tratamiento) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tratamiento->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->tratamiento->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->tratamiento->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaludtratamiento_tratamiento', range: false, from: '<?php echo $Page->tratamiento->RangeFrom; ?>', to: '<?php echo $Page->tratamiento->RangeTo; ?>', url: 'viewsaludtratamientorpt.php' });" id="x_tratamiento<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
...
<?php } ?>
<?php if ($Page->tratamiento->Visible) { ?>
		<td data-field="tratamiento"<?php echo $Page->tratamiento->CellAttributes() ?>>
<span<?php echo $Page->tratamiento->ViewAttributes() ?>><?php echo $Page->tratamiento->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "viewsaludtratamientorptpager.php" ?>
