...
<?php include_once "municipiorptinfo.php" ?>
<?php
...
$municipio_rpt = NULL; // Initialize page object first

class crmunicipio_rpt extends crmunicipio {
	...
	function __construct() {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'municipio', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fmunicipiorpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'municipio');
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fmunicipiorpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fmunicipiorpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fmunicipiorpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['id_provincia'] = ewr_Conv($rs->fields('id_provincia'), 3);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			...
			$this->Val[3] = $this->descripcion->CurrentValue;
			$this->Val[4] = $this->id_provincia->CurrentValue;
		} else {
			$this->id->setDbValue("");
			$this->nombre->setDbValue("");
			$this->descripcion->setDbValue("");
			$this->id_provincia->setDbValue("");
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->id_provincia->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// id_provincia
			$CurrentValue = $this->id_provincia->CurrentValue;
			$ViewValue = &$this->id_provincia->ViewValue;
			$ViewAttrs = &$this->id_provincia->ViewAttrs;
			$CellAttrs = &$this->id_provincia->CellAttrs;
			$HrefValue = &$this->id_provincia->HrefValue;
			$LinkAttrs = &$this->id_provincia->LinkAttrs;
			$this->Cell_Rendered($this->id_provincia, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->id_provincia->Visible) $this->DtlColumnCount += 1;
	}
...
// Create page object
if (!isset($municipio_rpt)) $municipio_rpt = new crmunicipio_rpt();
...
// Create page object
var municipio_rpt = new ewr_Page("municipio_rpt");

// Page properties
municipio_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = municipio_rpt.PageID;
</script>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_municipio" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id"><div class="municipio_id"><span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id">
<?php if ($Page->SortUrl($Page->id) == "") { ?>
		<div class="ewTableHeaderBtn municipio_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer municipio_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombre"><div class="municipio_nombre"><span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombre">
<?php if ($Page->SortUrl($Page->nombre) == "") { ?>
		<div class="ewTableHeaderBtn municipio_nombre">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer municipio_nombre" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombre) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="descripcion"><div class="municipio_descripcion"><span class="ewTableHeaderCaption"><?php echo $Page->descripcion->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="descripcion">
<?php if ($Page->SortUrl($Page->descripcion) == "") { ?>
		<div class="ewTableHeaderBtn municipio_descripcion">
			<span class="ewTableHeaderCaption"><?php echo $Page->descripcion->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer municipio_descripcion" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->descripcion) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->descripcion->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->id_provincia->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id_provincia"><div class="municipio_id_provincia"><span class="ewTableHeaderCaption"><?php echo $Page->id_provincia->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id_provincia">
<?php if ($Page->SortUrl($Page->id_provincia) == "") { ?>
		<div class="ewTableHeaderBtn municipio_id_provincia">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_provincia->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer municipio_id_provincia" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id_provincia) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_provincia->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->id_provincia->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->id_provincia->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php } ?>
<?php if ($Page->id_provincia->Visible) { ?>
		<td data-field="id_provincia"<?php echo $Page->id_provincia->CellAttributes() ?>>
<span<?php echo $Page->id_provincia->ViewAttributes() ?>><?php echo $Page->id_provincia->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "municipiorptpager.php" ?>
