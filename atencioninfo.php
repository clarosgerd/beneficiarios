...
class catencion extends cTable {
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(COALESCE(`nombre`, ''),'" . ew_ValueSeparator(1, $this->id_neonato) . "',COALESCE(`apellidopaterno`,''),'" . ew_ValueSeparator(2, $this->id_neonato) . "',COALESCE(`apellidomaterno`,'')) FROM `neonatal` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `atencion`.`id_neonato` LIMIT 1) AS `EV__id_neonato`, (SELECT CONCAT(COALESCE(`nombres`, ''),'" . ew_ValueSeparator(1, $this->id_escolar) . "',COALESCE(`apellidopaterno`,''),'" . ew_ValueSeparator(2, $this->id_escolar) . "',COALESCE(`apellidomaterno`,'')) FROM `escolar` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `atencion`.`id_escolar` LIMIT 1) AS `EV__id_escolar`, (SELECT CONCAT(COALESCE(`nombres`, ''),'" . ew_ValueSeparator(1, $this->id_especialista) . "',COALESCE(`apellidopaterno`,''),'" . ew_ValueSeparator(2, $this->id_especialista) . "',COALESCE(`apellidomaterno`,'')) FROM `especialista` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `atencion`.`id_especialista` LIMIT 1) AS `EV__id_especialista` FROM `atencion`" .
			") `EW_TMP_TABLE`";
