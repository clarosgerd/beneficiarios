...
class cotrasorganizaciones extends cTable {
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT DISTINCT CONCAT(`nombre`,'" . ew_ValueSeparator(1, $this->id_persona) . "',`apellidopaterno`,'" . ew_ValueSeparator(2, $this->id_persona) . "',`apellidomaterno`) FROM `persona` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `otrasorganizaciones`.`id_persona` LIMIT 1) AS `EV__id_persona` FROM `otrasorganizaciones`" .
			") `EW_TMP_TABLE`";
