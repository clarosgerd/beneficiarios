...
$otrasorganizaciones = NULL;

//
// Table class for otrasorganizaciones
//
class cotrasorganizaciones extends cTable {
	var $id;
	var $nombre;
	var $departamento;
	var $ciudad;
	...
	function __construct() {
		...
		$this->TableVar = 'otrasorganizaciones';
		$this->TableName = 'otrasorganizaciones';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`otrasorganizaciones`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// nombre
		$this->nombre = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nombre->Sortable = TRUE; // Allow sort
		$this->fields['nombre'] = &$this->nombre;

		// departamento
		$this->departamento = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x_departamento', 'departamento', '`departamento`', '`departamento`', 200, -1, FALSE, '`departamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->departamento->Sortable = TRUE; // Allow sort
		$this->fields['departamento'] = &$this->departamento;

		// ciudad
		$this->ciudad = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x_ciudad', 'ciudad', '`ciudad`', '`ciudad`', 200, -1, FALSE, '`ciudad`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->ciudad->Sortable = TRUE; // Allow sort
		$this->ciudad->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->ciudad->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->ciudad->OptionCount = 9;
		$this->fields['ciudad'] = &$this->ciudad;

		// direccion
		$this->direccion = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x_direccion', 'direccion', '`direccion`', '`direccion`', 200, -1, FALSE, '`direccion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->direccion->Sortable = TRUE; // Allow sort
		$this->fields['direccion'] = &$this->direccion;

		// telefono
		$this->telefono = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x_telefono', 'telefono', '`telefono`', '`telefono`', 200, -1, FALSE, '`telefono`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->telefono->Sortable = TRUE; // Allow sort
		$this->fields['telefono'] = &$this->telefono;

		// email
		$this->_email = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x__email', 'email', '`email`', '`email`', 200, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// id_persona
		$this->id_persona = new cField('otrasorganizaciones', 'otrasorganizaciones', 'x_id_persona', 'id_persona', '`id_persona`', '`id_persona`', 3, -1, FALSE, '`EV__id_persona`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`otrasorganizaciones`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT DISTINCT CONCAT(COALESCE(`nombre`, ''),'" . ew_ValueSeparator(1, $this->id_persona) . "',COALESCE(`apellidopaterno`,''),'" . ew_ValueSeparator(2, $this->id_persona) . "',COALESCE(`apellidomaterno`,'')) FROM `persona` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id` = `otrasorganizaciones`.`id_persona` LIMIT 1) AS `EV__id_persona` FROM `otrasorganizaciones`" .
			") `EW_TMP_TABLE`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "otrasorganizacioneslist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "otrasorganizacionesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "otrasorganizacionesadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("otrasorganizacionesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("otrasorganizacionesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "otrasorganizacionesadd.php?" . $this->UrlParm($parm);
		else
			$url = "otrasorganizacionesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("otrasorganizacionesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("otrasorganizacionesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("otrasorganizacionesdelete.php", $this->UrlParm());
	}
