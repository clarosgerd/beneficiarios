...
$referencia_delete = NULL; // Initialize page object first

class creferencia_delete extends creferencia {
	...
	// Page object name
	var $PageObjName = 'referencia_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("referencialist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in referencia class, referenciainfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("referencialist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($referencia_delete)) $referencia_delete = new creferencia_delete();

// Page init
$referencia_delete->Page_Init();

// Page main
$referencia_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$referencia_delete->Page_Render();
?>
...
var CurrentForm = freferenciadelete = new ew_Form("freferenciadelete", "delete");

// Form_CustomValidate event
freferenciadelete.Form_CustomValidate = 
...
freferenciadelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
freferenciadelete.Lists["x_id_medio"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"medio"};
freferenciadelete.Lists["x_id_medio"].Data = "<?php echo $referencia_delete->id_medio->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $referencia_delete->ShowPageHeader(); ?>
<?php
$referencia_delete->ShowMessage();
?>
<form name="freferenciadelete" id="freferenciadelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($referencia_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $referencia_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="referencia">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($referencia_delete->RecKeys as $key) { ?>
...
<?php if ($referencia->id->Visible) { // id ?>
		<th class="<?php echo $referencia->id->HeaderCellClass() ?>"><span id="elh_referencia_id" class="referencia_id"><?php echo $referencia->id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($referencia->id_medio->Visible) { // id_medio ?>
		<th class="<?php echo $referencia->id_medio->HeaderCellClass() ?>"><span id="elh_referencia_id_medio" class="referencia_id_medio"><?php echo $referencia->id_medio->FldCaption() ?></span></th>
<?php } ?>
<?php if ($referencia->nombrescompleto->Visible) { // nombrescompleto ?>
		<th class="<?php echo $referencia->nombrescompleto->HeaderCellClass() ?>"><span id="elh_referencia_nombrescompleto" class="referencia_nombrescompleto"><?php echo $referencia->nombrescompleto->FldCaption() ?></span></th>
<?php } ?>
...
<?php } ?>
	...
	<tbody>
<?php
$referencia_delete->RecCnt = 0;
$i = 0;
while (!$referencia_delete->Recordset->EOF) {
	$referencia_delete->RecCnt++;
	$referencia_delete->RowCnt++;

	// Set row properties
	$referencia->ResetAttrs();
	$referencia->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$referencia_delete->LoadRowValues($referencia_delete->Recordset);

	// Render row
	$referencia_delete->RenderRow();
?>
	<tr<?php echo $referencia->RowAttributes() ?>>
<?php if ($referencia->id->Visible) { // id ?>
		<td<?php echo $referencia->id->CellAttributes() ?>>
<span id="el<?php echo $referencia_delete->RowCnt ?>_referencia_id" class="referencia_id">
...
<?php } ?>
<?php if ($referencia->id_medio->Visible) { // id_medio ?>
		<td<?php echo $referencia->id_medio->CellAttributes() ?>>
<span id="el<?php echo $referencia_delete->RowCnt ?>_referencia_id_medio" class="referencia_id_medio">
...
<?php } ?>
<?php if ($referencia->nombrescompleto->Visible) { // nombrescompleto ?>
		<td<?php echo $referencia->nombrescompleto->CellAttributes() ?>>
<span id="el<?php echo $referencia_delete->RowCnt ?>_referencia_nombrescompleto" class="referencia_nombrescompleto">
...
<?php } ?>
<?php if ($referencia->nombrescentromedico->Visible) { // nombrescentromedico ?>
		<td<?php echo $referencia->nombrescentromedico->CellAttributes() ?>>
<span id="el<?php echo $referencia_delete->RowCnt ?>_referencia_nombrescentromedico" class="referencia_nombrescentromedico">
...
<?php } ?>
<?php if ($referencia->telefono->Visible) { // telefono ?>
		<td<?php echo $referencia->telefono->CellAttributes() ?>>
<span id="el<?php echo $referencia_delete->RowCnt ?>_referencia_telefono" class="referencia_telefono">
...
<?php
	$referencia_delete->Recordset->MoveNext();
}
$referencia_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $referencia_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
freferenciadelete.Init();
</script>
<?php
$referencia_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$referencia_delete->Page_Terminate();
?>
