...
<?php include_once "inscripcionestudianterptinfo.php" ?>
<?php
...
$inscripcionestudiante_rpt = NULL; // Initialize page object first

class crinscripcionestudiante_rpt extends crinscripcionestudiante {
	...
	// Page object name
	var $PageObjName = 'inscripcionestudiante_rpt';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["inscripcionestudiante"])) {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'inscripcionestudiante', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption finscripcionestudianterpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'inscripcionestudiante');
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"finscripcionestudianterpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"finscripcionestudianterpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"finscripcionestudianterpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['id_estudiante'] = ewr_Conv($rs->fields('id_estudiante'), 3);
				$this->FirstRowData['id_curso'] = ewr_Conv($rs->fields('id_curso'), 3);
				$this->FirstRowData['id_gestion'] = ewr_Conv($rs->fields('id_gestion'), 3);
		...
		if (!$rs->EOF) {
			...
			$this->Val[1] = $this->id_estudiante->CurrentValue;
			$this->Val[2] = $this->id_curso->CurrentValue;
			$this->Val[3] = $this->id_gestion->CurrentValue;
			$this->Val[4] = $this->fecha->CurrentValue;
			$this->Val[5] = $this->esincritoespecial->CurrentValue;
		} else {
			$this->id_estudiante->setDbValue("");
			$this->id_curso->setDbValue("");
			$this->id_gestion->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->id_estudiante->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// id_curso
			$this->id_curso->ViewValue = $this->id_curso->CurrentValue;
			$this->id_curso->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// id_gestion
			$this->id_gestion->ViewValue = $this->id_gestion->CurrentValue;
			$this->id_gestion->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// id_estudiante
			$CurrentValue = $this->id_estudiante->CurrentValue;
			$ViewValue = &$this->id_estudiante->ViewValue;
			$ViewAttrs = &$this->id_estudiante->ViewAttrs;
			$CellAttrs = &$this->id_estudiante->CellAttrs;
			$HrefValue = &$this->id_estudiante->HrefValue;
			$LinkAttrs = &$this->id_estudiante->LinkAttrs;
			$this->Cell_Rendered($this->id_estudiante, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// id_curso
			$CurrentValue = $this->id_curso->CurrentValue;
			$ViewValue = &$this->id_curso->ViewValue;
			$ViewAttrs = &$this->id_curso->ViewAttrs;
			$CellAttrs = &$this->id_curso->CellAttrs;
			$HrefValue = &$this->id_curso->HrefValue;
			$LinkAttrs = &$this->id_curso->LinkAttrs;
			$this->Cell_Rendered($this->id_curso, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// id_gestion
			$CurrentValue = $this->id_gestion->CurrentValue;
			$ViewValue = &$this->id_gestion->ViewValue;
			$ViewAttrs = &$this->id_gestion->ViewAttrs;
			$CellAttrs = &$this->id_gestion->CellAttrs;
			$HrefValue = &$this->id_gestion->HrefValue;
			$LinkAttrs = &$this->id_gestion->LinkAttrs;
			$this->Cell_Rendered($this->id_gestion, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->id_estudiante->Visible) $this->DtlColumnCount += 1;
		if ($this->id_curso->Visible) $this->DtlColumnCount += 1;
		if ($this->id_gestion->Visible) $this->DtlColumnCount += 1;
...
// Create page object
if (!isset($inscripcionestudiante_rpt)) $inscripcionestudiante_rpt = new crinscripcionestudiante_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$inscripcionestudiante_rpt;
...
// Create page object
var inscripcionestudiante_rpt = new ewr_Page("inscripcionestudiante_rpt");

// Page properties
inscripcionestudiante_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = inscripcionestudiante_rpt.PageID;
</script>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_inscripcionestudiante" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<table class="<?php echo $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->id_estudiante->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id_estudiante"><div class="inscripcionestudiante_id_estudiante"><span class="ewTableHeaderCaption"><?php echo $Page->id_estudiante->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id_estudiante">
<?php if ($Page->SortUrl($Page->id_estudiante) == "") { ?>
		<div class="ewTableHeaderBtn inscripcionestudiante_id_estudiante">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_estudiante->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer inscripcionestudiante_id_estudiante" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id_estudiante) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_estudiante->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->id_estudiante->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->id_estudiante->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->id_curso->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id_curso"><div class="inscripcionestudiante_id_curso"><span class="ewTableHeaderCaption"><?php echo $Page->id_curso->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id_curso">
<?php if ($Page->SortUrl($Page->id_curso) == "") { ?>
		<div class="ewTableHeaderBtn inscripcionestudiante_id_curso">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_curso->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer inscripcionestudiante_id_curso" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id_curso) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_curso->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->id_curso->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->id_curso->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->id_gestion->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="id_gestion"><div class="inscripcionestudiante_id_gestion"><span class="ewTableHeaderCaption"><?php echo $Page->id_gestion->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="id_gestion">
<?php if ($Page->SortUrl($Page->id_gestion) == "") { ?>
		<div class="ewTableHeaderBtn inscripcionestudiante_id_gestion">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_gestion->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer inscripcionestudiante_id_gestion" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->id_gestion) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->id_gestion->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->id_gestion->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->id_gestion->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="fecha"><div class="inscripcionestudiante_fecha"><span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="fecha">
<?php if ($Page->SortUrl($Page->fecha) == "") { ?>
		<div class="ewTableHeaderBtn inscripcionestudiante_fecha">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer inscripcionestudiante_fecha" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->fecha) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="esincritoespecial"><div class="inscripcionestudiante_esincritoespecial"><span class="ewTableHeaderCaption"><?php echo $Page->esincritoespecial->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="esincritoespecial">
<?php if ($Page->SortUrl($Page->esincritoespecial) == "") { ?>
		<div class="ewTableHeaderBtn inscripcionestudiante_esincritoespecial">
			<span class="ewTableHeaderCaption"><?php echo $Page->esincritoespecial->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer inscripcionestudiante_esincritoespecial" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->esincritoespecial) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->esincritoespecial->FldCaption() ?></span>
...
<?php if ($Page->id_estudiante->Visible) { ?>
		<td data-field="id_estudiante"<?php echo $Page->id_estudiante->CellAttributes() ?>>
<span<?php echo $Page->id_estudiante->ViewAttributes() ?>><?php echo $Page->id_estudiante->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->id_curso->Visible) { ?>
		<td data-field="id_curso"<?php echo $Page->id_curso->CellAttributes() ?>>
<span<?php echo $Page->id_curso->ViewAttributes() ?>><?php echo $Page->id_curso->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->id_gestion->Visible) { ?>
		<td data-field="id_gestion"<?php echo $Page->id_gestion->CellAttributes() ?>>
<span<?php echo $Page->id_gestion->ViewAttributes() ?>><?php echo $Page->id_gestion->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->fecha->Visible) { ?>
...
<?php include "inscripcionestudianterptpager.php" ?>
