...
class cpersona extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('persona', 'persona', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// nombreinstitucion
		$this->nombreinstitucion = new cField('persona', 'persona', 'x_nombreinstitucion', 'nombreinstitucion', '`nombreinstitucion`', '`nombreinstitucion`', 200, -1, FALSE, '`nombreinstitucion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nombreinstitucion->Sortable = TRUE; // Allow sort
		$this->fields['nombreinstitucion'] = &$this->nombreinstitucion;

		// apellidopaterno
		$this->apellidopaterno = new cField('persona', 'persona', 'x_apellidopaterno', 'apellidopaterno', '`apellidopaterno`', '`apellidopaterno`', 200, -1, FALSE, '`apellidopaterno`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->apellidopaterno->Sortable = TRUE; // Allow sort
		$this->fields['apellidopaterno'] = &$this->apellidopaterno;

		// apellidomaterno
		$this->apellidomaterno = new cField('persona', 'persona', 'x_apellidomaterno', 'apellidomaterno', '`apellidomaterno`', '`apellidomaterno`', 200, -1, FALSE, '`apellidomaterno`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// telefono
		$this->telefono = new cField('persona', 'persona', 'x_telefono', 'telefono', '`telefono`', '`telefono`', 200, -1, FALSE, '`telefono`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tipopersona
		$this->tipopersona = new cField('persona', 'persona', 'x_tipopersona', 'tipopersona', '`tipopersona`', '`tipopersona`', 200, -1, FALSE, '`tipopersona`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tipopersona->Sortable = TRUE; // Allow sort
		$this->fields['tipopersona'] = &$this->tipopersona;
	}
	...
	function RenderEditRow() {
		...
		$this->tipopersona->EditValue = $this->tipopersona->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->tipopersona->Exportable) $Doc->ExportCaption($this->tipopersona);
				} else {
					if ($this->id->Exportable) $Doc->ExportCaption($this->id);
					if ($this->nombreinstitucion->Exportable) $Doc->ExportCaption($this->nombreinstitucion);
					if ($this->apellidopaterno->Exportable) $Doc->ExportCaption($this->apellidopaterno);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->tipopersona->Exportable) $Doc->ExportField($this->tipopersona);
					} else {
						if ($this->id->Exportable) $Doc->ExportField($this->id);
						if ($this->nombreinstitucion->Exportable) $Doc->ExportField($this->nombreinstitucion);
						if ($this->apellidopaterno->Exportable) $Doc->ExportField($this->apellidopaterno);
