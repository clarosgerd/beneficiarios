...
<?php include_once "viewsaludmarcologicosummaryrptinfo.php" ?>
<?php
...
$viewsaludmarcologicosummary_rpt = NULL; // Initialize page object first

class crviewsaludmarcologicosummary_rpt extends crviewsaludmarcologicosummary {
	...
	// Page object name
	var $PageObjName = 'viewsaludmarcologicosummary_rpt';
	...
	function __construct() {
		...
		// Table object (viewsaludmarcologicosummary)
		if (!isset($GLOBALS["viewsaludmarcologicosummary"])) {
			$GLOBALS["viewsaludmarcologicosummary"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["viewsaludmarcologicosummary"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'viewsaludmarcologicosummary', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fviewsaludmarcologicosummaryrpt";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'viewsaludmarcologicosummary');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_viewsaludmarcologicosummary\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_viewsaludmarcologicosummary',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fviewsaludmarcologicosummaryrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fviewsaludmarcologicosummaryrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fviewsaludmarcologicosummaryrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['cuadro11'] = ewr_Conv($rs->fields('cuadro11'), 131);
				$this->FirstRowData['cuadro12'] = ewr_Conv($rs->fields('cuadro12'), 131);
				$this->FirstRowData['cuadro13'] = ewr_Conv($rs->fields('cuadro13'), 131);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'viewsaludmarcologicosummary_nombreinstitucion') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'viewsaludmarcologicosummary_fecha') {
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_viewsaludmarcologicosummary_nombreinstitucion"])) {
			$this->LoadSelectionFromSession('nombreinstitucion');
		} elseif (@$_SESSION["sel_viewsaludmarcologicosummary_nombreinstitucion"] == EWR_INIT_VALUE) { // Select all
			$this->nombreinstitucion->SelectionList = "";
		}

		// Get fecha selected values
		if (is_array(@$_SESSION["sel_viewsaludmarcologicosummary_fecha"])) {
			$this->LoadSelectionFromSession('fecha');
		} elseif (@$_SESSION["sel_viewsaludmarcologicosummary_fecha"] == EWR_INIT_VALUE) { // Select all
			$this->fecha->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->PopupName == 'viewsaludmarcologicosummary_nombreinstitucion' && $this->PopupValue <> "") {
			...
			} elseif ($this->ClearExtFilter == 'viewsaludmarcologicosummary_nombreinstitucion') {
				$this->SetSessionDropDownValue(EWR_INIT_VALUE, '', 'nombreinstitucion');
			}

			// Set/clear dropdown for field fecha
			if ($this->PopupName == 'viewsaludmarcologicosummary_fecha' && $this->PopupValue <> "") {
				if ($this->PopupValue == EWR_INIT_VALUE)
					$this->fecha->DropDownValue = EWR_ALL_VALUE;
				else
					$this->fecha->DropDownValue = $this->PopupValue;
				$bRestoreSession = FALSE; // Do not restore
			} elseif ($this->ClearExtFilter == 'viewsaludmarcologicosummary_fecha') {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->nombreinstitucion)) {
				$bSetupFilter = TRUE;
			} elseif ($this->nombreinstitucion->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_viewsaludmarcologicosummary_nombreinstitucion'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->fecha)) {
				$bSetupFilter = TRUE;
			} elseif ($this->fecha->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_viewsaludmarcologicosummary_fecha'])) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_viewsaludmarcologicosummary_nombreinstitucion'] = ($this->nombreinstitucion->SelectionList == "") ? EWR_INIT_VALUE : $this->nombreinstitucion->SelectionList;
			...
			$_SESSION['sel_viewsaludmarcologicosummary_fecha'] = ($this->fecha->SelectionList == "") ? EWR_INIT_VALUE : $this->fecha->SelectionList;
		}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_viewsaludmarcologicosummary_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_viewsaludmarcologicosummary_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_viewsaludmarcologicosummary_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_viewsaludmarcologicosummary_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_viewsaludmarcologicosummary_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_viewsaludmarcologicosummary_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_viewsaludmarcologicosummary_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_viewsaludmarcologicosummary_' . $parm] = $sv;
		$_SESSION['so_viewsaludmarcologicosummary_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_viewsaludmarcologicosummary_' . $parm] = $sv1;
		$_SESSION['so_viewsaludmarcologicosummary_' . $parm] = $so1;
		$_SESSION['sc_viewsaludmarcologicosummary_' . $parm] = $sc;
		$_SESSION['sv2_viewsaludmarcologicosummary_' . $parm] = $sv2;
		$_SESSION['so2_viewsaludmarcologicosummary_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_viewsaludmarcologicosummary_$parm"] = "";
		$_SESSION["rf_viewsaludmarcologicosummary_$parm"] = "";
		$_SESSION["rt_viewsaludmarcologicosummary_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_viewsaludmarcologicosummary_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_viewsaludmarcologicosummary_$parm"];
		$fld->RangeTo = @$_SESSION["rt_viewsaludmarcologicosummary_$parm"];
	}
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_nombreinstitucion", $filter)) {
			...
			$_SESSION["sel_viewsaludmarcologicosummary_nombreinstitucion"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "nombreinstitucion");
			$this->nombreinstitucion->SelectionList = "";
			$_SESSION["sel_viewsaludmarcologicosummary_nombreinstitucion"] = "";
		}
		...
		if (array_key_exists("sel_fecha", $filter)) {
			...
			$_SESSION["sel_viewsaludmarcologicosummary_fecha"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "fecha");
			$this->fecha->SelectionList = "";
			$_SESSION["sel_viewsaludmarcologicosummary_fecha"] = "";
		}
		return TRUE;
	}
	...
	function GetDrillDownFilter() {
		...
		if ($opt == "1" || $opt == "2") {
			...
			$_SESSION['do_viewsaludmarcologicosummary'] = $opt;
			$_SESSION['df_viewsaludmarcologicosummary'] = $filter;
			$_SESSION['dl_viewsaludmarcologicosummary'] = $sFilterList;
		} elseif (@$_GET["cmd"] == "resetdrilldown") { // Clear drill down
			$_SESSION[EWR_PROJECT_NAME . "_" . $this->TableVar . "_" . EWR_TABLE_MASTER_TABLE] = "";
			$_SESSION['do_viewsaludmarcologicosummary'] = "";
			$_SESSION['df_viewsaludmarcologicosummary'] = "";
			$_SESSION['dl_viewsaludmarcologicosummary'] = "";
		} else { // Restore from Session
			$opt = @$_SESSION['do_viewsaludmarcologicosummary'];
			$filter = @$_SESSION['df_viewsaludmarcologicosummary'];
			$sFilterList = @$_SESSION['dl_viewsaludmarcologicosummary'];
		}
		if ($opt == "1" || $opt == "2")
			$this->DrillDown = TRUE;
		if ($opt == "1") {
	...
	function GetSort($options = array()) {
		...
		// Check for an Order parameter
			...
			$this->UpdateSort($this->cuadro11); // cuadro11
			$this->UpdateSort($this->cuadro12); // cuadro12
			$this->UpdateSort($this->cuadro13); // cuadro13
...
// Create page object
if (!isset($viewsaludmarcologicosummary_rpt)) $viewsaludmarcologicosummary_rpt = new crviewsaludmarcologicosummary_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$viewsaludmarcologicosummary_rpt;
...
// Create page object
var viewsaludmarcologicosummary_rpt = new ewr_Page("viewsaludmarcologicosummary_rpt");

// Page properties
viewsaludmarcologicosummary_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = viewsaludmarcologicosummary_rpt.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fviewsaludmarcologicosummaryrpt = new ewr_Form("fviewsaludmarcologicosummaryrpt");

// Validate method
fviewsaludmarcologicosummaryrpt.Validate = function() {
...
fviewsaludmarcologicosummaryrpt.Form_CustomValidate = 
...
fviewsaludmarcologicosummaryrpt.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fviewsaludmarcologicosummaryrpt.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fviewsaludmarcologicosummaryrpt.Lists["sv_nombreinstitucion"] = {"LinkField":"sv_nombreinstitucion","Ajax":true,"DisplayFields":["sv_nombreinstitucion","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fviewsaludmarcologicosummaryrpt.AutoSuggests["sv_nombreinstitucion"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $viewsaludmarcologicosummary_rpt->nombreinstitucion->LookupFilterQuery(TRUE))) ?>;
fviewsaludmarcologicosummaryrpt.Lists["sv_fecha"] = {"LinkField":"sv_fecha","Ajax":true,"DisplayFields":["sv_fecha","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="fviewsaludmarcologicosummaryrpt" id="fviewsaludmarcologicosummaryrpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fviewsaludmarcologicosummaryrpt_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<script type="text/javascript">
fviewsaludmarcologicosummaryrpt.Lists["sv_nombreinstitucion"].Options = <?php echo ewr_ArrayToJson($Page->nombreinstitucion->LookupFilterOptions) ?>;
</script>
</span>
</div>
...
<select data-table="viewsaludmarcologicosummary" data-field="x_fecha" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->fecha->DisplayValueSeparator) ? json_encode($Page->fecha->DisplayValueSeparator) : $Page->fecha->DisplayValueSeparator) ?>" id="sv_fecha" name="sv_fecha"<?php echo $Page->fecha->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
...
<script type="text/javascript">
fviewsaludmarcologicosummaryrpt.Lists["sv_fecha"].Options = <?php echo ewr_ArrayToJson($Page->fecha->LookupFilterOptions) ?>;
</script>
</span>
</div>
</div>
...
<script type="text/javascript">
fviewsaludmarcologicosummaryrpt.Init();
fviewsaludmarcologicosummaryrpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_viewsaludmarcologicosummary" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombreinstitucion"><div class="viewsaludmarcologicosummary_nombreinstitucion"><span class="ewTableHeaderCaption"><?php echo $Page->nombreinstitucion->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombreinstitucion">
<?php if ($Page->SortUrl($Page->nombreinstitucion) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_nombreinstitucion">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreinstitucion->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaludmarcologicosummary_nombreinstitucion', range: false, from: '<?php echo $Page->nombreinstitucion->RangeFrom; ?>', to: '<?php echo $Page->nombreinstitucion->RangeTo; ?>', url: 'viewsaludmarcologicosummaryrpt.php' });" id="x_nombreinstitucion<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_nombreinstitucion" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombreinstitucion) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombreinstitucion->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="fecha"><div class="viewsaludmarcologicosummary_fecha"><span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="fecha">
<?php if ($Page->SortUrl($Page->fecha) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_fecha">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'viewsaludmarcologicosummary_fecha', range: false, from: '<?php echo $Page->fecha->RangeFrom; ?>', to: '<?php echo $Page->fecha->RangeTo; ?>', url: 'viewsaludmarcologicosummaryrpt.php' });" id="x_fecha<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_fecha" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->fecha) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro1"><div class="viewsaludmarcologicosummary_cuadro1"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro1->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro1">
<?php if ($Page->SortUrl($Page->cuadro1) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro1">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro1->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro1" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro1) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro1->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro2"><div class="viewsaludmarcologicosummary_cuadro2"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro2->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro2">
<?php if ($Page->SortUrl($Page->cuadro2) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro2">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro2->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro2" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro2) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro2->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro3"><div class="viewsaludmarcologicosummary_cuadro3"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro3->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro3">
<?php if ($Page->SortUrl($Page->cuadro3) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro3">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro3->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro3" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro3) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro3->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro4"><div class="viewsaludmarcologicosummary_cuadro4"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro4->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro4">
<?php if ($Page->SortUrl($Page->cuadro4) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro4">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro4->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro4" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro4) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro4->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro5"><div class="viewsaludmarcologicosummary_cuadro5"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro5->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro5">
<?php if ($Page->SortUrl($Page->cuadro5) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro5">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro5->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro5" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro5) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro5->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro6"><div class="viewsaludmarcologicosummary_cuadro6"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro6->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro6">
<?php if ($Page->SortUrl($Page->cuadro6) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro6">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro6->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro6" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro6) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro6->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro7"><div class="viewsaludmarcologicosummary_cuadro7"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro7->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro7">
<?php if ($Page->SortUrl($Page->cuadro7) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro7">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro7->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro7" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro7) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro7->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro8"><div class="viewsaludmarcologicosummary_cuadro8"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro8->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro8">
<?php if ($Page->SortUrl($Page->cuadro8) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro8">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro8->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro8" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro8) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro8->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro9"><div class="viewsaludmarcologicosummary_cuadro9"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro9->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro9">
<?php if ($Page->SortUrl($Page->cuadro9) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro9">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro9->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro9" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro9) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro9->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro10"><div class="viewsaludmarcologicosummary_cuadro10"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro10->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro10">
<?php if ($Page->SortUrl($Page->cuadro10) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro10">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro10->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro10" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro10) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro10->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro11"><div class="viewsaludmarcologicosummary_cuadro11"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro11->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro11">
<?php if ($Page->SortUrl($Page->cuadro11) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro11">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro11->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro11" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro11) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro11->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro12"><div class="viewsaludmarcologicosummary_cuadro12"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro12->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro12">
<?php if ($Page->SortUrl($Page->cuadro12) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro12">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro12->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro12" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro12) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro12->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cuadro13"><div class="viewsaludmarcologicosummary_cuadro13"><span class="ewTableHeaderCaption"><?php echo $Page->cuadro13->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cuadro13">
<?php if ($Page->SortUrl($Page->cuadro13) == "") { ?>
		<div class="ewTableHeaderBtn viewsaludmarcologicosummary_cuadro13">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro13->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer viewsaludmarcologicosummary_cuadro13" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cuadro13) ?>',1);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cuadro13->FldCaption() ?></span>
...
<?php include "viewsaludmarcologicosummaryrptpager.php" ?>
